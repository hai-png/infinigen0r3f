...
export class YOLOExporter {
  ...
  private categories: YOLOCategory[] = [];
  
  /** Collected images */
  private images: YOLOImage[] = [];
  
  /** Current YOLO version */
  private yoloVersion: 'v5' | 'v8' | 'v11' = 'v8';

  constructor(yoloVersion: 'v5' | 'v8' | 'v11' = 'v8') {
  ...
  registerCategory(name: string): number {
    if (!this.categoryMap.has(name)) {
      ...
      const category: YOLOCategory = { id, name };
  ...
  addImage(options: {
    ...
    for (const bbox of options.bboxes) {
      ...
      if (
        normalizedBbox.xCenter >= 0 && normalizedBbox.xCenter <= 1 &&
   ...
   */
  splitDatasets(ratios: [number, number, number] = [0.8, 0.1, 0.1], seed: number = 42): {
    train: YOLOImage[];
    val: YOLOImage[];
    test: YOLOImage[];
  } {
  ...
  generateLabelContent(image: YOLOImage): string {
    ...
    for (const bbox of image.bboxes) {
      ...
      if (
        bbox.xCenter >= 0 && bbox.xCenter <= 1 &&
        bbox.yCenter >= 0 && bbox.yCenter <= 1 &&
        bbox.width > 0 && bbox.width <= 1 &&
        bbox.height > 0 && bbox.height <= 1
      ) {
        lines.push(
          `${bbox.classId} ${bbox.xCenter.toFixed(6)} ${bbox.yCenter.toFixed(6)} ${bbox.width.toFixed(6)} ${bbox.height.toFixed(6)}`
        );
      }
    }
   ...
   */
  async export(config: YOLOExportConfig): Promise<{
    trainCount: number;
    valCount: number;
    testCount: number;
  }> {
   ...
   */
  private async writeLabelsToDisk(
    images: YOLOImage[],
    outputDir: string,
    fs: any,
    path: any
   ...
   */
  private convertToYAML(config: YOLODatasetConfig): string {
    const lines: string[] = [];
