...
export interface DollyShotConfig {
  ...
  duration?: number;
  target?: THREE.Vector3;        // Point to look at (optional)
  easeIn?: boolean;              // Smooth start
  easeOut?: boolean;             // Smooth end
  fovStart?: number;             // Starting FOV
  fovEnd?: number;               // Ending FOV (for dolly zoom effect)
 ...
 */
export function createDollyShot(config: DollyShotConfig): Keyframe[] {
  const {
    start,
    end,
    duration = 3,
 ...
 */
export function generateDollyTrajectory(config: DollyShotConfig): TrajectorySample[] {
 ...
 */
export function createPullBack(
  startPosition: THREE.Vector3,
 ...
 */
export function createTrackingShot(
  start: THREE.Vector3,
  end: THREE.Vector3,
