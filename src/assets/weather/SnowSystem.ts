...
export class SnowSystem {
  ...
  update(deltaTime: number): void {
    ...
    const positions = this.snowMesh.geometry.attributes.position.array as Float32Array;
   ...
   */
  private addAccumulation(x: number, z: number, deltaTime: number): void {
   ...
   */
  createAccumulationMeshes(gridSize: number = 2): void {
    if (!this.params.accumulationEnabled) return;
