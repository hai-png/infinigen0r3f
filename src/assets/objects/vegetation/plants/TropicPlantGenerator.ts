...
export interface TropicPlantConfig {
  ...
  // Size parameters
  height: number;           // Overall plant height (0.5 - 8.0)
  stemRadius: number;       // Stem/petiole thickness (0.01 - 0.3)
  leafSize: number;         // Leaf size multiplier (0.5 - 3.0)
  
  // Leaf properties
  leafCount: number;        // Number of leaves (3 - 30)
  leafSplitDepth: number;   // Depth of leaf splits (0.0 - 1.0)
  leafFenestration: number; // Holes in leaves (0.0 - 1.0)
  leafWaviness: number;     // Edge wave amplitude (0.0 - 1.0)
  leafDroop: number;        // Downward droop (0.0 - 1.0)
  
  // Color and pattern
  primaryColor: THREE.Color;
  secondaryColor: THREE.Color;
  variegation: number;      // Color variegation pattern (0.0 - 1.0)
  glossiness: number;       // Leaf surface shine (0.0 - 1.0)
  
  // Growth pattern
  spiralAngle: number;      // Angle between successive leaves (radians)
  internodeLength: number;  // Distance between leaves (0.05 - 0.5)
  
  // Environmental
  humidity: number;         // Affects leaf size and droop (0.0 - 1.0)
  lightExposure: number;    // Affects color intensity (0.0 - 1.0)
}
...
export class TropicPlantGenerator {
   ...
   */
  private generateLeaf(stemPosition: number, angle: number): THREE.Group {
    const group = new THREE.Group();
   ...
   * Create petiole (leaf stalk)
   */
  private createPetiole(): THREE.Mesh {
   ...
   * Create leaf blade based on species
   */
  private createLeafBlade(): THREE.Mesh {
   ...
   */
  private createMonsteraLeaf(): THREE.BufferGeometry {
   ...
   */
  private createBirdOfParadiseLeaf(): THREE.BufferGeometry {
   ...
   */
  private createBananaLeaf(): THREE.BufferGeometry {
   ...
   */
  private createHeartShapedLeaf(): THREE.BufferGeometry {
   ...
   */
  private createArrowLeaf(): THREE.BufferGeometry {
    const { leafSize } = this.config;
   ...
   */
  private createPalmateLeaf(): THREE.BufferGeometry {
    const { leafSize, leafSplitDepth } = this.config;
   ...
   */
  private createGenericLeaf(): THREE.BufferGeometry {
    const { leafSize } = this.config;
   ...
   */
  private applyLeafSplits(geometry: THREE.BufferGeometry, depth: number): void {
    const positions = geometry.attributes.position.array;
   ...
   */
  private addFenestrations(geometry: THREE.BufferGeometry, density: number): void {
   ...
   */
  private applyEdgeWave(geometry: THREE.BufferGeometry, amplitude: number): void {
    const positions = geometry.attributes.position.array;
   ...
   */
  private generateAerialRoots(): THREE.Group {
    const group = new THREE.Group();
   ...
   */
  private getConfigWithDefaults(): TropicPlantConfig & { segments: number } {
    return {
      ...this.config,
      segments: 8,
    };
