...
export interface JointState {
  ...
  localRotation: number;  // Local joint angle
}
...
export class CCDIKSolver {
  ...
  private targetOrientation?: Vector3;
   ...
   */
  private forwardKinematics(): void {
...
export class FABRIKSolver {
  ...
  solve(): number {
    ...
    while (iterations < this.maxIterations && !converged) {
      ...
      for (let i = n - 2; i >= 0; i--) {
        ...
        this.joints[i].position.lerpVectors(
          this.joints[i + 1].position,
   ...
   */
  private applyConstraints(index: number): void {
    ...
    if (index > 0) {
      ...
      if (clampedAngle !== angle) {
        ...
        joint.position.set(
          parent.position.x + length * Math.cos(clampedAngle),
          parent.position.y + length * Math.sin(clampedAngle),
          0
        );
      }
    }
   ...
   */
  private calculateRotations(): void {
...
export class InverseKinematics {
  private solver: CCDIKSolver | FABRIKSolver | null;
  private solverType: IKSolverType;
  private config: IKChainConfig;
   ...
   */
  private createSolver(): CCDIKSolver | FABRIKSolver {
   ...
   */
  getEndEffectorPosition(): Vector3 {
    if (!this.solver) return new Vector3();
    
    if (this.solver instanceof CCDIKSolver) {
      return this.solver.getEndEffectorPosition();
    } else {
      return (this.solver as FABRIKSolver).getEndEffectorPosition();
    }
  }
  ...
  setSolverType(type: IKSolverType): void {
    if (this.solverType === type) return;
  ...
  reset(): void {
    if (this.solver) {
      if (this.solver instanceof CCDIKSolver) {
        ...
        (this.solver as FABRIKSolver).reset();
      }
    }
  }
}
...
export function createArmChain(
  upperArmLength: number = 1,
  forearmLength: number = 1,
  handLength: number = 0.5
): IKChainConfig {
  return {
    joints: [
...
export function createLegChain(
  thighLength: number = 1,
  shinLength: number = 1,
  footLength: number = 0.5
): IKChainConfig {
  return {
    joints: [
 ...
 */
export function createSnakeChain(
  segments: number = 10,
