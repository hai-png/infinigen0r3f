...
export interface OscillatoryConfig {
  ...
  waveType?: WaveType;
  axis?: 'x' | 'y' | 'z' | Vector3;
  decay?: number; // Damping factor (0 = no decay, >0 = exponential decay)
  offset?: Vector3; // Base position offset
}
...
export interface PatternConfig {
  ...
  scale?: number;
  speed?: number;
  noiseScale?: number; // For noise-based patterns
  damping?: number; // For damped oscillations
  center?: Vector3; // Center point for patterns
}
...
export class OscillatoryMotion {
  private config: OscillatoryConfig;
  private time: number;
...
export class PatternGenerator {
  ...
  getPosition(): Vector3 {
    ...
    switch (this.config.type) {
      ...
      default:
        ...
        const waveFunc = (amp: number, freq: number, phase: number) => 
          evaluateWave(this.config.type as WaveType, t, amp, freq, phase) * dampingFactor;
   ...
   */
  getVelocity(deltaTime: number = 0.016): Vector3 {
   ...
   */
  private noise1D(t: number): number {
 ...
 */
export function createPresetMotion(
  preset: 'bounce' | 'float' | 'vibrate' | 'pulse' | 'swing',
  options: Partial<OscillatoryConfig> = {}
): OscillatoryMotion {
  const presets: Record<string, OscillatoryConfig> = {
    bounce: {
      amplitude: 1,
      frequency: 2,
      waveType: 'sine',
...
export function createPresetPattern(
  preset: 'orbit' | 'figure8' | 'helix' | 'random',
