...
export interface OrbitShotConfig {
  ...
  radius: number;
  startAngle?: number;      // Horizontal start angle (radians)
  endAngle?: number;        // Horizontal end angle (radians)
  elevation?: number;       // Vertical angle (radians, 0 = horizon, PI/2 = top)
  minElevation?: number;    // Minimum elevation for arc
  maxElevation?: number;    // Maximum elevation for arc
  rotations?: number;       // Number of full rotations
  duration?: number;        // Total duration in seconds
  clockwise?: boolean;      // Direction of orbit
  lookAtCenter?: boolean;   // Always look at center point
 ...
 */
export function createOrbitShot(config: OrbitShotConfig): Keyframe[] {
  const {
    center,
    radius,
    startAngle = 0,
 ...
 */
export function generateOrbitTrajectory(config: OrbitShotConfig): TrajectorySample[] {
 ...
 */
export function createSpiralOrbit(
  center: THREE.Vector3,
  startRadius: number,
  endRadius: number,
  duration: number = 5,
