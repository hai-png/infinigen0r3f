...
export interface MemoryBreakdown {
  ...
  /** Largest geometries */
  largestGeometries: Array<{ name: string; size: number }>;
  /** Largest textures */
  largestTextures: Array<{ name: string; size: number }>;
}
...
export class MemoryProfiler {
  ...
  getStats(): MemoryStats {
    ...
    const geometrySizes: Array<{ name: string; size: number }> = [];
    const textureSizes: Array<{ name: string; size: number }> = [];
      ...
      if (object instanceof THREE.Mesh) {
        ...
        if (mesh.geometry && !seenGeometries.has(mesh.geometry)) {
          ...
          if (this.config.detailedTracking) {
            ...
            breakdown.byGeometryType.set(
              typeName,
              (breakdown.byGeometryType.get(typeName) || 0) + geomSize
            );
        ...
        materials.forEach(material => {
          if (material && !seenMaterials.has(material)) {
            ...
            if (this.config.detailedTracking) {
              ...
              breakdown.byMaterialType.set(
                typeName,
                (breakdown.byMaterialType.get(typeName) || 0) + matSize
              );
            }
          }
   ...
   */
  private extractTextures(material: THREE.Material): THREE.Texture[] {
   ...
   */
  private generateWarnings(stats: MemoryStats): string[] {
    ...
    if (stats.totalHeapSize > 0) {
      ...
      if (heapUsage > this.config.heapWarningThreshold) {
        warnings.push(
          `High heap usage: ${(heapUsage * 100).toFixed(1)}% (${this.formatBytes(stats.usedHeapSize)} / ${this.formatBytes(stats.totalHeapSize)})`
        );
      }
    }
    ...
    if (textureMB > this.config.textureWarningThreshold) {
      warnings.push(
        `High texture memory: ${textureMB.toFixed(1)} MB (threshold: ${this.config.textureWarningThreshold} MB)`
      );
    }
    ...
    if (geometryMB > this.config.geometryWarningThreshold) {
      warnings.push(
        `High geometry memory: ${geometryMB.toFixed(1)} MB (threshold: ${this.config.geometryWarningThreshold} MB)`
      );
    }
