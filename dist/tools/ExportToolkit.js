...
export class ExportToolkit {
    ...
    async exportPLY(objects, options) {
    ...
    async exportThreeJSON(scene, options) {
    ...
    getObjectsToExport(scene, options) {
        const objects = [];
        if (options.selectedOnly && options.selectedIds) {
            ...
            scene.traverse((object) => {
                if (object instanceof Mesh || object.type === 'Group')
                    objects.push(object);
            });
        }
        return objects;
...
export function createExportToolkit(onProgress) {
    return new ExportToolkit(onProgress);
}
export default ExportToolkit;
//# sourceMappingURL=ExportToolkit.js.map