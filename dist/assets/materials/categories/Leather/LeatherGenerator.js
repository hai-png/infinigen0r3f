...
export class LeatherGenerator extends BaseMaterialGenerator {
    constructor() { super(); }
    getDefaultParams() { return { ...LeatherGenerator.DEFAULT_PARAMS }; }
    ...
    generateRoughnessMap(params, rng) {
        const size = 256;
        const canvas = document.createElement('canvas');
        canvas.width = size;
        canvas.height = size;
    ...
    getVariations(count) {
        const variations = [];
        const types = ['full-grain', 'top-grain', 'suede', 'distressed', 'patent'];
...
LeatherGenerator.DEFAULT_PARAMS = {
    type: 'full-grain',
...
//# sourceMappingURL=LeatherGenerator.js.map