...
export class MosaicMaterial {
     ...
     * Generate mosaic texture based on pattern type
     */
    static generateMosaicTexture(ctx, size, config) {
     ...
     */
    static addTileVariation(ctx, x, y, width, height, baseColor, config) {
     ...
     */
    static addTileDetail(ctx, x, y, width, height, config) {
        ...
        // Bottom-right shadow
        const shadowGradient = ctx.createLinearGradient(x + width - bevelSize, y + height - bevelSize, x + width, y + height);
        ...
        for (let y = 0; y < size; y++) {
            for (let x = 0; x < size; x++) {
                ...
                if (!inGrout) {
                    ...
                    const edgeDistance = Math.min(localX, localY, tilePixelSize - localX, tilePixelSize - localY);
...
//# sourceMappingURL=MosaicMaterial.js.map