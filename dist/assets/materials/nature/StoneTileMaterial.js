...
export class StoneTileMaterial {
     ...
     */
    static generateTileTexture(ctx, size, config) {
     ...
     * Generate roughness map
     */
    static generateRoughnessMap(ctx, size, config) {
     ...
     */
    static addCrack(ctx, x, y, width, height, config) {
     ...
     */
    static addWeathering(ctx, size, config) {
        const imageData = ctx.getImageData(0, 0, size, size);
        const data = imageData.data;
        for (let y = 0; y < size; y++) {
            for (let x = 0; x < size; x++) {
...
StoneTileMaterial.DEFAULT_CONFIG = {
...
//# sourceMappingURL=StoneTileMaterial.js.map