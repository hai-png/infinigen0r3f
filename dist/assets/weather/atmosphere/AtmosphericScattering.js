...
export class AtmosphericScattering {
    ...
    updateRayleighCoefficients() {
        if (!this.skyMaterial)
            return;
        const wavelength = new THREE.Vector3(680e-9, 550e-9, 450e-9);
        const invWavelength = new THREE.Vector3(1.0 / Math.pow(wavelength.x, 4), 1.0 / Math.pow(wavelength.y, 4), 1.0 / Math.pow(wavelength.z, 4));
        const rayleigh = invWavelength.multiplyScalar((8 * Math.pow(Math.PI, 3) * Math.pow(0.000293 - 1, 2)) /
            (3 * 10000.0 * 0.000293));
    ...
    updateClouds() {
        ...
        if (this.cloudConfig.coverage < 0.01)
            return;
...
//# sourceMappingURL=AtmosphericScattering.js.map