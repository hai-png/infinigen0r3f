...
export class CloudLayer {
    constructor(type = 'cumulus', params = {}) {
        this.type = type;
...
export class VolumetricClouds {
        ...
        for (let z = 0; z < size3D; z++) {
            for (let y = 0; y < size3D; y++) {
                for (let x = 0; x < size3D; x++) {
                    ...
                    data3D[idx] = this.generatePerlinNoise3D(x / size3D * 4, y / size3D * 4, z / size3D * 4);
                }
            }
        }
    ...
    generatePerlinNoise3D(x, y, z) {
        ...
        return this.lerp(w, this.lerp(v, this.lerp(u, this.grad(AA, x, y, z), this.grad(BA, x - 1, y, z)), this.lerp(u, this.grad(AB, x, y - 1, z), this.grad(BB, x - 1, y - 1, z))), this.lerp(v, this.lerp(u, this.grad(AA + 1, x, y, z - 1), this.grad(BA + 1, x - 1, y, z - 1)), this.lerp(u, this.grad(AB + 1, x, y - 1, z - 1), this.grad(BB + 1, x - 1, y - 1, z - 1))));
    }
    generatePerlinNoise2D(x, y) {
        const X = Math.floor(x) & 255;
        const Y = Math.floor(y) & 255;
        x -= Math.floor(x);
        y -= Math.floor(y);
        ...
        const A = this.p[X] + Y;
        const B = this.p[X + 1] + Y;
        return this.lerp(v, this.lerp(u, this.grad2D(A, x, y), this.grad2D(B, x - 1, y)), this.lerp(u, this.grad2D(A + 1, x, y - 1), this.grad2D(B + 1, x - 1, y - 1)));
    }
    fade(t) {
...
//# sourceMappingURL=VolumetricClouds.js.map