...
export class GaitGenerator {
     ...
     */
    calculatePhaseOffset(legIndex, totalLegs, gaitType) {
        switch (gaitType) {
            case 'walk':
    ...
    update(deltaTime) {
        ...
        for (const legState of this.legStates) {
            ...
            if (legState.isStance) {
                ...
                legState.currentPosition.set(legConfig.restPosition.x + xOffset, legConfig.restPosition.y, legConfig.restPosition.z);
                ...
                legState.currentPosition.set(legConfig.restPosition.x + xLerp, legConfig.restPosition.y + liftHeight, legConfig.restPosition.z);
            }
        }
    }
    /**
    ...
    setGaitType(gaitType) {
        ...
        for (let i = 0; i < this.legStates.length; i++) {
            ...
            legState.config.phaseOffset = this.calculatePhaseOffset(i, this.legStates.length, gaitType);
        }
    }
    /**
     ...
     */
    getBodyHeightOffset(terrainHeights) {
        let totalOffset = 0;
        let count = 0;
        for (const legState of this.legStates) {
            if (legState.isStance) {
     ...
     */
    static createQuadrupedConfig(bodyLength = 1, bodyWidth = 0.5, legLength = 0.5) {
        return [
            {
                name: 'front_left',
     ...
     */
    static createHexapodConfig(bodyLength = 1, bodyWidth = 0.4, legLength = 0.4) {
        const legs = [];
        const segments = 3;
     ...
     */
    static createBipedConfig(hipWidth = 0.3, legLength = 1) {
        return [
            {
                name: 'left_leg',
 ...
 */
export function createPresetGait(creatureType, gaitType = 'walk', options = {}) {
    let legs;
    switch (creatureType) {
        case 'quadruped':
            legs = GaitGenerator.createQuadrupedConfig(options.bodyLength, options.bodyWidth, options.legLength);
            break;
        case 'hexapod':
            legs = GaitGenerator.createHexapodConfig(options.bodyLength, options.bodyWidth, options.legLength);
            break;
        case 'biped':
            legs = GaitGenerator.createBipedConfig(options.bodyWidth, options.legLength);
            break;
        default:
            legs = [];
    }
...
//# sourceMappingURL=GaitGenerator.js.map