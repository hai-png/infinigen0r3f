...
export class DeskGenerator extends BaseObjectGenerator {
    getDefaultConfig() {
        return {
            width: 1.4,
            depth: 0.7,
    ...
    createDrawers(params, rng) {
        ...
        for (let i = 0; i < params.drawerCount; i++) {
            ...
            drawer.position.set(params.width / 2 - drawerWidth / 2 - 0.05, 0.05 + i * drawerHeight + drawerHeight / 2, 0);
            ...
            handle.position.set(params.width / 2 - 0.02, 0.05 + i * drawerHeight + drawerHeight / 2, drawerDepth / 2 + 0.01);
            drawerGroup.add(handle);
        }
        return drawerGroup;
    }
    createHutch(params, rng) {
    ...
    createCableManagement(params, rng) {
    ...
    getVariationCount() { return 4 * 4 * 2 * 2 * 2; }
    register() { ObjectRegistry.register(DeskGenerator.GENERATOR_ID, this); }
}
DeskGenerator.GENERATOR_ID = 'desk_generator';
if (typeof window !== 'undefined') {
    new DeskGenerator().register();
}
export default DeskGenerator;
//# sourceMappingURL=DeskGenerator.js.map