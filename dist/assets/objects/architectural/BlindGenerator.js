...
export class BlindGenerator {
     ...
     */
    getDefaultSlatThickness(type, material) {
     ...
     */
    createMount(params) {
        const geometry = new THREE.BoxGeometry(params.width + 0.04, 0.05, 0.08);
    ...
    createVerticalBlinds(params) {
        ...
        for (let i = 0; i < vaneCount; i++) {
            const vane = this.createSlat(params, params.slatWidth, params.height, params.slatThickness);
    ...
    createShutters(params) {
        ...
        for (let p = 0; p < panelCount; p++) {
            ...
            for (let i = 0; i < louverCount; i++) {
                const louver = this.createSlat(params, panelWidth - 0.04, params.slatWidth, params.slatThickness);
     ...
     */
    createSlat(params, width, height, thickness) {
     ...
     */
    createLadderTapes(params, slatCount) {
        const group = new THREE.Group();
        const tapeCount = 3;
     ...
     */
    getFabricMaterial(color, materialType) {
        return new THREE.MeshStandardMaterial({
            color,
            roughness: 0.8,
            metalness: 0,
     ...
     */
    generateSet(windowCount, windowWidth, windowHeight, spacing, params = {}) {
        const group = new THREE.Group();
...
//# sourceMappingURL=BlindGenerator.js.map