 ...
 */
export var UrchinType;
(function (UrchinType) {
    UrchinType["REGULAR"] = "regular";
    UrchinType["LONG_SPINED"] = "long_spined";
    UrchinType["PENCIL"] = "pencil";
    UrchinType["FIRE"] = "fire";
    UrchinType["COLLECTOR"] = "collector";
})(UrchinType || (UrchinType = {}));
/**
...
export class UrchinGenerator {
     ...
     */
    static createBodyGeometry(config) {
     ...
     */
    static getSpineGeometry(config) {
        ...
        switch (config.type) {
            case UrchinType.LONG_SPINED:
                // Long, thin spines
                geometry = this.createTaperedCylinder(config.spineLength, config.spineThickness * 0.3, config.spineThickness, 8);
                break;
            case UrchinType.PENCIL:
                // Thick, blunt spines
                geometry = this.createTaperedCylinder(config.spineLength, config.spineThickness * 0.8, config.spineThickness, 10);
                break;
            case UrchinType.FIRE:
                // Spines with barbs (simplified as segmented)
                geometry = this.createBarbedSpine(config.spineLength, config.spineThickness, 6);
                break;
            case UrchinType.COLLECTOR:
                // Short, thick spines
                geometry = this.createTaperedCylinder(config.spineLength * 0.7, config.spineThickness * 0.6, config.spineThickness * 1.2, 8);
                break;
            default:
                // Regular urchin - medium spines
                geometry = this.createTaperedCylinder(config.spineLength, config.spineThickness * 0.4, config.spineThickness, 8);
        }
     ...
     */
    static createTaperedCylinder(height, topRadius, baseRadius, radialSegments) {
        const geometry = new THREE.CylinderGeometry(topRadius, baseRadius, height, radialSegments, 1, false);
     ...
     */
    static createBarbedSpine(height, baseThickness, segments) {
        const points = [];
     ...
     */
    static getSpineMaterial(config) {
     ...
     */
    static generateCluster(config, count, area) {
        const group = new THREE.Group();
        for (let i = 0; i < count; i++) {
            const urchin = this.generateUrchin(config);
            // Random position
            urchin.position.set((Math.random() - 0.5) * area.width, config.bodySize * 0.5, (Math.random() - 0.5) * area.depth);
            // Random rotation
...
UrchinGenerator.materialCache = new Map();
UrchinGenerator.spineGeometryCache = new Map();
//# sourceMappingURL=UrchinGenerator.js.map