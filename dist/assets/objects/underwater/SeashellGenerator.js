 ...
 */
export var SeashellType;
(function (SeashellType) {
    SeashellType["CLAM"] = "clam";
    SeashellType["SCALLOP"] = "scallop";
    SeashellType["CONCH"] = "conch";
    SeashellType["NAUTILUS"] = "nautilus";
    SeashellType["COWRIE"] = "cowrie";
    SeashellType["MUSSEL"] = "mussel";
    SeashellType["OYSTER"] = "oyster";
    SeashellType["COCKLE"] = "cockle";
...
export class SeashellGenerator {
     ...
     */
    static createShellGeometry(config) {
        switch (config.type) {
     ...
     */
    static createBivalveShell(config, isClam) {
     ...
     */
    static createScallopShell(config) {
        const { size, damageLevel } = config;
     ...
     */
    static createSpiralShell(config, variant) {
        const { size, damageLevel } = config;
        const points = [];
     ...
     */
    static addConchFlare(geometry, size, endPoint) {
     ...
     */
    static createCowrieShell(config) {
        const { size } = config;
     ...
     */
    static createOysterShell(config) {
        const { size, damageLevel } = config;
     ...
     */
    static createCockleShell(config) {
        const { size, damageLevel } = config;
        // Heart-shaped base
        const shape = new THREE.Shape();
     ...
     */
    static generateScatter(types, count, area) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            // Position on seabed
            shell.position.set((Math.random() - 0.5) * area.width, 0, (Math.random() - 0.5) * area.depth);
            // Random rotation
            shell.rotation.set(Math.random() * 0.3, Math.random() * Math.PI * 2, Math.random() * 0.3);
            group.add(shell);
        }
        return group;
    }
...
SeashellGenerator.materialCache = new Map();
//# sourceMappingURL=SeashellGenerator.js.map