...
export class BenchGenerator {
    ...
    createSeat(params) {
        ...
        if (params.type === 'picnic') {
            ...
            for (let i = 0; i < plankCount; i++) {
                ...
                plank.position.set(-params.length / 2 + params.length / 2, params.seatHeight, -params.width / 2 + i * plankWidth + plankWidth / 2);
                group.add(plank);
            }
        }
        else {
            ...
            for (let i = 0; i < params.slatCount; i++) {
                ...
                slat.position.set(0, params.seatHeight, -params.width / 2 + i * spacing);
                group.add(slat);
            }
        }
     ...
     */
    createSlat(params, length, width) {
    ...
    createSeatFrame(params) {
        ...
        for (let i = 0; i < crossCount; i++) {
            ...
            cross.position.set(-params.length / 2 + params.length / (crossCount + 1) * (i + 1), params.seatHeight - 0.05, 0);
            group.add(cross);
        }
        return group;
    }
    ...
    createBackrest(params) {
        ...
        for (let i = 0; i < backrestSlats; i++) {
            ...
            slat.position.set(0, params.seatHeight + 0.1 + i * 0.1, -params.width / 2 - 0.03);
     ...
     */
    createStretcher(params, legCount) {
    ...
    createPicnicTable(params) {
        ...
        for (let i = 0; i < params.slatCount + 2; i++) {
            ...
            plank.position.set(0, tableHeight, -params.width / 2 + i * 0.12);
     ...
     */
    createStorageBox(params) {
        const geometry = new THREE.BoxGeometry(params.length - 0.1, 0.3, params.width - 0.1);
        const material = this.getMaterial(params.material, params.secondaryColor);
     ...
     */
    generateRow(count, spacing, params = {}) {
        const group = new THREE.Group();
...
//# sourceMappingURL=BenchGenerator.js.map