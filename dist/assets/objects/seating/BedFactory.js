 ...
 */
export class BedFactory extends AssetFactory {
    constructor(seed) {
        super(seed);
        this.sheetTypes = ['quilt', 'comforter', 'box_comforter', 'none'];
        this.mattressTypes = ['coiled', 'wrapped'];
        this.legDecorTypes = ['coiled', 'pad', 'plain', 'legs'];
        this.pillowTypes = ['standard', 'king', 'body'];
    }
    /**
    ...
    createFrame(config) {
        ...
        const platformGeometry = new THREE.BoxGeometry(config.width, config.thickness, config.size);
        ...
        for (let i = 0; i < (config.hasAllLegs ? 4 : 2); i++) {
            ...
            const legGeometry = new THREE.BoxGeometry(config.legThickness, config.legHeight, config.legThickness);
        ...
        // Create headboard
        const headboardGeometry = new THREE.BoxGeometry(config.width, config.backHeight, config.thickness);
     ...
     * Create pillows
     */
    createPillows(config) {
        const group = new THREE.Group();
        const materials = [];
        const pillowDimensions = this.getPillowDimensions(config.pillowType);
        const material = this.createFabricMaterial();
        for (let i = 0; i < config.pillowCount; i++) {
            const geometry = new THREE.BoxGeometry(pillowDimensions[0], pillowDimensions[1], pillowDimensions[2]);
            ...
            pillow.position.set(x, config.legHeight + config.thickness + 0.25 + pillowDimensions[1] / 2, -config.size * 0.35);
    ...
    createBedding(config) {
        ...
        switch (config.sheetType) {
            case 'quilt':
                geometry = new THREE.BoxGeometry(config.width * 1.5, 0.05, config.size * 1.0);
                break;
            case 'comforter':
                geometry = new THREE.BoxGeometry(config.width * 1.6, 0.08, config.size * 1.1);
                break;
            case 'box_comforter':
                geometry = new THREE.BoxGeometry(config.width * 1.7, 0.1, config.size * 1.15);
                break;
            default:
        ...
        if (config.hasCover && config.sheetType !== 'none') {
            const coverGeometry = new THREE.BoxGeometry(config.width * 1.7, 0.03, config.size * 0.35);
            const cover = new THREE.Mesh(coverGeometry, material);
            cover.position.set(0, bedding.position.y + 0.06, bedding.position.z - config.size * 0.3);
            group.add(cover);
        }
        return {
            mesh: group,
     ...
     */
    decorateHeadboard(headboard, config) {
        // Add panel details
        if (config.seatSubdivisionsX > 1) {
            const panelGroup = new THREE.Group();
            for (let i = 0; i < config.seatSubdivisionsX; i++) {
                const panelGeometry = new THREE.BoxGeometry(config.width / config.seatSubdivisionsX - config.panelMargin, config.backHeight * 0.6, 0.02);
                const panel = new THREE.Mesh(panelGeometry, headboard.material);
                panel.position.set(-config.width / 2 + (config.width / config.seatSubdivisionsX) * (i + 0.5), config.backHeight * 0.3, config.thickness / 2 + 0.01);
     ...
     */
    addQuiltingPattern(mattress, config) {
