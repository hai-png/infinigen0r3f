...
export class VaseGenerator extends BaseObjectGenerator {
    ...
    getVariations(count = 4, baseConfig) {
        ...
        for (let i = 0; i < count; i++) {
            configs.push({
                ...this.defaultParams,
                shape: ['cylinder', 'amphora', 'bud', 'hourglass', 'flute'][Math.floor(Math.random() * 5)],
                material: ['ceramic', 'clay', 'glass', 'crystal', 'porcelain'][Math.floor(Math.random() * 5)],
                color: '#' + Math.floor(Math.random() * 16777215).toString(16),
                hasHandles: Math.random() > 0.5,
                handleCount: Math.floor(Math.random() * 3) + 1,
                surfaceFinish: ['glossy', 'matte', 'textured'][Math.floor(Math.random() * 3)],
                rimStyle: ['straight', 'flared', 'rolled'][Math.floor(Math.random() * 3)],
                pattern: ['none', 'stripes', 'floral'][Math.floor(Math.random() * 3)],
