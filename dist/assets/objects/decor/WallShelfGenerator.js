...
export class WallShelfGenerator {
     ...
     */
    createShelfBoard() {
        const geometry = new THREE.BoxGeometry(this.config.width, this.config.thickness, this.config.depth);
    ...
    addEdgeDetail(shelf) {
        // Create edge molding
        const edgeGeometry = new THREE.BoxGeometry(this.config.width + 0.02, 0.02, this.config.depth + 0.02);
     ...
     */
    addLedge(group, shelfY) {
        const ledgeHeight = 0.05;
        const ledgeGeo = new THREE.BoxGeometry(this.config.width, ledgeHeight, 0.03);
    ...
    createBackPanel() {
        ...
        const geometry = new THREE.BoxGeometry(this.config.width + 0.04, totalHeight, 0.02);
        const material = this.getMaterial();
        const mesh = new THREE.Mesh(geometry, material);
    ...
    addSidePanels(group) {
        const totalHeight = (this.config.tiers - 1) * this.config.tierSpacing + this.config.thickness;
        const sideGeometry = new THREE.BoxGeometry(0.03, totalHeight, this.config.depth);
        const material = this.getMaterial();
        // Left side
...
//# sourceMappingURL=WallShelfGenerator.js.map