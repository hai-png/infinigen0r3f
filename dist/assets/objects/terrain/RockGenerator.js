...
export class RockGenerator {
    ...
    createRockGeometry() {
        ...
        geometry.scale(this.config.width * this.config.size, this.config.height * this.config.size, this.config.depth * this.config.size);
    ...
    displaceVertices(geometry) {
        ...
        for (let i = 0; i < positionAttribute.count; i++) {
            ...
            const noiseValue = this.layeredNoise(vertex.x * this.config.noiseScale, vertex.y * this.config.noiseScale, vertex.z * this.config.noiseScale, this.config.octaves);
            // Apply irregularity
    ...
    generateCluster(count, spread) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            rockGeo.scale(scale * this.config.width * this.config.size, scale * this.config.height * this.config.size, scale * this.config.depth * this.config.size);
     ...
     */
    generateCliffFace(width, height, depth) {
     ...
     */
    layeredNoise(x, y, z, octaves) {
        let value = 0;
        let amplitude = 1;
        let frequency = 1;
        let maxValue = 0;
        for (let i = 0; i < octaves; i++) {
            value += amplitude * this.noise.perlin3D(x * frequency, y * frequency, z * frequency);
            maxValue += amplitude;
            amplitude *= 0.5;
            frequency *= 2;
        }
