...
export class CliffGenerator {
    ...
    createCliffGeometry() {
        ...
        const geometry = new THREE.BoxGeometry(this.config.width, this.config.height, this.config.depth, this.config.segments * 2, this.config.segments * this.config.layerCount, this.config.segments);
    ...
    applyLayers(geometry) {
        ...
        for (let i = 0; i < positionAttribute.count; i++) {
            ...
            const layerNoise = this.noise.perlin2D(vertex.x * 0.1, vertex.z * 0.1);
     ...
     */
    addFractures(geometry) {
        const positionAttribute = geometry.attributes.position;
        const vertex = new THREE.Vector3();
        for (let i = 0; i < positionAttribute.count; i++) {
            vertex.fromBufferAttribute(positionAttribute, i);
            // Only modify front/back faces significantly
            if (Math.abs(vertex.z) < this.config.depth * 0.3)
                continue;
            // Calculate fracture noise
            const fractureNoise = this.noise.perlin3D(vertex.x * this.config.fractureDensity * 2, vertex.y * 0.5, vertex.z * this.config.fractureDensity * 2);
    ...
    applyErosion(geometry) {
        ...
        for (let i = 0; i < positionAttribute.count; i++) {
            ...
            // Apply erosion noise
            const erosionNoise = this.noise.perlin2D(vertex.x * 0.2, vertex.z * 0.2);
     ...
     */
    generateCliffFormation(count, spacing) {
...
//# sourceMappingURL=CliffGenerator.js.map