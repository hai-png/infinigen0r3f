...
export class SmallPlantGenerator {
    ...
    addSucculentFlowers(group, config) {
    ...
    addCactusArms(group, stemHeight, stemRadius, material) {
    ...
    addAloeTeeth(leaf, length, material) {
        const toothCount = 5;
    ...
    createSpiderPlant(group, config) {
        const leafMaterial = new THREE.MeshStandardMaterial({
            color: config.leafColor,
            roughness: 0.5,
            metalness: 0.0,
    ...
    addSpiderPlantlets(group, config) {
        ...
        for (let i = 0; i < plantletCount; i++) {
            ...
            plantletGroup.position.set(Math.cos(angle) * runnerLength, 0.02, Math.sin(angle) * runnerLength);
            group.add(plantletGroup);
        }
    }
}
//# sourceMappingURL=SmallPlantGenerator.js.map