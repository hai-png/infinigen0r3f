 ...
 */
export const TreeSpeciesPresets = {
    oak: {
        name: 'Oak',
...
export class TreeGenerator {
     ...
     */
    generateTree(species, seed, options = {}) {
     ...
     */
    generateBranches(config, seed, lod) {
        const branchesGroup = new THREE.Group();
        const branchCount = Math.floor(this.randomInRange(config.branchCount.min, config.branchCount.max, seed));
     ...
     */
    createIrregularCrown(baseRadius, height, seed, lod) {
     ...
     * Create palm fronds
     */
    createPalmFronds(radius, height, seed, lod) {
        const frondCount = 8;
        const geometries = [];
     ...
     */
    generateSnowCap(config, seed, lod) {
     ...
     */
    generateForest(count, areaSize, speciesList, seed, options = {}) {
