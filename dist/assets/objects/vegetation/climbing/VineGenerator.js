...
export class VineGenerator {
    ...
    generateClimbingStems(config) {
        ...
        for (let i = 0; i < stemCount; i++) {
            ...
            const points = [];
            let currentPos = new THREE.Vector3((Math.random() - 0.5) * 0.3, 0, (Math.random() - 0.5) * 0.3);
    ...
    generateHangingStems(config) {
        ...
        for (let i = 0; i < stemCount; i++) {
            ...
            for (let s = 0; s < segments; s++) {
                ...
                currentPos = new THREE.Vector3(currentPos.x + swayAmount * 0.1, currentPos.y - config.length / segments, currentPos.z + (Math.random() - 0.5) * 0.02);
                points.push(currentPos.clone());
            }
            const curve = new THREE.CatmullRomCurve3(points);
    ...
    generateLeaves(stems, config) {
        ...
        stems.forEach((stem, stemIndex) => {
            ...
            for (let i = 0; i < leafCount; i++) {
                ...
                point.set((Math.random() - 0.5) * config.length * 0.3, t * config.length, (Math.random() - 0.5) * config.length * 0.3);
                // Create leaf
                ...
                leaf.rotation.set((Math.random() - 0.5) * 0.5, Math.random() * Math.PI * 2, (Math.random() - 0.5) * 0.5);
                leaves.push(leaf);
            }
        });
        return leaves;
    ...
    generateFlowers(stems, config) {
        ...
        for (let i = 0; i < flowerCount; i++) {
            ...
            flower.position.set((Math.random() - 0.5) * config.length * 0.3, Math.random() * config.length, (Math.random() - 0.5) * config.length * 0.3);
            flowers.push(flower);
        }
        return flowers;
    }
     ...
     */
    createLeafGeometry(species, size) {
        switch (species) {
            case 'ivy':
    ...
    createWisteriaLeafGeometry(size) {
    ...
    createGrapevineLeafGeometry(size) {
...
//# sourceMappingURL=VineGenerator.js.map