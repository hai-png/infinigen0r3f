...
export class ClothingGenerator {
    ...
    createShirtGeometry() {
        ...
        for (let i = 0; i < leftSleevePos.length; i += 3) {
            positions.push(leftSleevePos[i] - bodyWidth / 2 - sleeveLength / 2, leftSleevePos[i + 1] + bodyHeight / 4, leftSleevePos[i + 2]);
        }
        const rightSleevePos = rightSleeveGeometry.attributes.position.array;
        for (let i = 0; i < rightSleevePos.length; i += 3) {
            positions.push(rightSleevePos[i] + bodyWidth / 2 + sleeveLength / 2, rightSleevePos[i + 1] + bodyHeight / 4, rightSleevePos[i + 2]);
        }
    ...
    createPantsGeometry() {
        ...
        for (let i = 0; i < leftLegPos.length; i += 3) {
            positions.push(leftLegPos[i] - waistWidth / 4, leftLegPos[i + 1] - length / 4, leftLegPos[i + 2]);
        }
        const rightLegPos = rightLegGeometry.attributes.position.array;
        for (let i = 0; i < rightLegPos.length; i += 3) {
            positions.push(rightLegPos[i] + waistWidth / 4, rightLegPos[i + 1] - length / 4, rightLegPos[i + 2]);
        }
        const geometry = new THREE.BufferGeometry();
        geometry.setAttribute('position', new THREE.Float32BufferAttribute(positions, 3));
        geometry.computeVertexNormals();
    ...
    createDressGeometry() {
        ...
        for (let i = 0; i < skirtPos.length; i += 3) {
            positions.push(skirtPos[i], skirtPos[i + 1] - bodiceHeight / 2 - skirtLength / 2, skirtPos[i + 2]);
        }
        const geometry = new THREE.BufferGeometry();
        geometry.setAttribute('position', new THREE.Float32BufferAttribute(positions, 3));
        geometry.computeVertexNormals();
    ...
    createJacketGeometry() {
        ...
        for (let i = 0; i < collarPos.length; i += 3) {
            positions.push(collarPos[i], collarPos[i + 1] + bodyHeight / 2, collarPos[i + 2]);
        }
        const geometry = new THREE.BufferGeometry();
        geometry.setAttribute('position', new THREE.Float32BufferAttribute(positions, 3));
        geometry.computeVertexNormals();
    ...
    createSocksGeometry() {
        ...
        for (let i = 0; i < anklePos.length; i += 3) {
            positions.push(anklePos[i], anklePos[i + 1] + footLength / 2 + ankleHeight / 2, anklePos[i + 2]);
        }
        const geometry = new THREE.BufferGeometry();
        geometry.setAttribute('position', new THREE.Float32BufferAttribute(positions, 3));
        geometry.computeVertexNormals();
     ...
     */
    applyPatternToMaterial(material) {
