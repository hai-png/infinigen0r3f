...
export class FabricDrape {
    ...
    createDrapeGeometry() {
        const segments = 32;
        const rows = 24;
        const geometry = new THREE.PlaneGeometry(this.config.width, this.config.height, segments, rows);
     ...
     */
    applyCurtainDrape(positions, segments, rows) {
     ...
     */
    applyTableclothDrape(positions, segments, rows) {
        const wrinkleIntensity = this.config.wrinkleIntensity || 0.5;
        ...
        for (let i = 0; i < positions.length; i += 3) {
            ...
            if (overEdgeX || overEdgeY) {
                const distFromEdge = Math.min(Math.abs(Math.abs(x) - tableWidth / 2), Math.abs(Math.abs(y) - tableDepth / 2));
                // Hang down
     ...
     */
    applyBlanketDrape(positions, segments, rows) {
        const wrinkleIntensity = this.config.wrinkleIntensity || 0.5;
        for (let i = 0; i < positions.length; i += 3) {
            const x = positions[i];
            const y = positions[i + 1];
     ...
     */
    applyTowelDrape(positions, segments, rows) {
        const wrinkleIntensity = this.config.wrinkleIntensity || 0.5;
        for (let i = 0; i < positions.length; i += 3) {
            const x = positions[i];
            const y = positions[i + 1];
     ...
     */
    applySheetDrape(positions, segments, rows) {
        const wrinkleIntensity = this.config.wrinkleIntensity || 0.5;
        for (let i = 0; i < positions.length; i += 3) {
            const x = positions[i];
            const y = positions[i + 1];
     ...
     */
    static createHangingCurtain(width, height, color, fabricType = 'curtain') {
     ...
     * Generate tablecloth
     */
    static createTablecloth(width, depth, color, fabricType = 'linen') {
        const drape = new FabricDrape({
            fabricType,
            width: width * 1.3,
            height: depth * 1.3,
     ...
     */
    static createFoldedTowel(width, height, color) {
...
//# sourceMappingURL=FabricDrape.js.map