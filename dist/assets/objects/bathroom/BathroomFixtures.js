 ...
 */
import { Group, BoxGeometry, CylinderGeometry, SphereGeometry, TorusGeometry, Mesh, ExtrudeGeometry, Shape } from 'three';
import { BaseObjectGenerator } from '../utils/BaseObjectGenerator';
import { BBox } from '../../../core/util/math/bbox';
export class BathroomFixtures extends BaseObjectGenerator {
    getDefaultConfig() {
    ...
    createUndermountBasin(size, style) {
        const material = this.getCeramicMaterial('white');
    ...
    createFreestandingTub(length, width, height, style) {
        const material = this.getCeramicMaterial('white');
        if (style === 'modern') {
    ...
    createAlcoveTub(length, width, height, isOval) {
        const material = this.getCeramicMaterial('white');
        if (isOval) {
    ...
    createShowerEnclosure(size, height, style) {
        const group = new Group();
        // Glass panels
    ...
    createWalkInShower(size, height, style) {
        const group = new Group();
        // Single glass panel
        const glassMat = this.createPBRMaterial({ color: 0x88ccff, metalness: 0.9, roughness: 0.05, transparent: true, opacity: 0.3 });
        const panelGeo = new BoxGeometry(size, height, 0.01);
    ...
    createTubShowerCombo(size, height, finish) {
        const group = new Group();
        // Tub base
    ...
    getBoundingBox(params) {
        const size = params.size === 'compact' ? 0.8 : params.size === 'large' ? 1.2 : 1.0;
        return new BBox({ x: -size / 2, y: 0, z: -size / 2 }, { x: size / 2, y: params.fixtureType === 'shower' ? 2.0 : 1.0, z: size / 2 });
    }
    ...
    getRandomParams() {
        const types = ['toilet', 'sink', 'bathtub', 'shower'];
        const styles = ['modern', 'traditional', 'minimal', 'luxury'];
        const finishes = ['white', 'black', 'stainless', 'colored'];
        const faucetStyles = ['single', 'double', 'wall', 'floor'];
        const tubShapes = ['rectangular', 'oval', 'corner', 'freestanding'];
        const showerTypes = ['enclosure', 'walk-in', 'tub-shower'];
...
//# sourceMappingURL=BathroomFixtures.js.map