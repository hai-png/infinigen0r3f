...
export class ApplianceBase extends BaseObjectGenerator {
    getDefaultConfig() {
    ...
    validateParams(params) {
        ...
        if (validated.width !== undefined)
            validated.width = Math.max(0.3, Math.min(1.2, validated.width));
        if (validated.height !== undefined)
            validated.height = Math.max(0.5, Math.min(2.0, validated.height));
        if (validated.depth !== undefined)
            validated.depth = Math.max(0.4, Math.min(0.9, validated.depth));
        if (validated.doorCount !== undefined)
            validated.doorCount = Math.max(1, Math.min(4, validated.doorCount));
        return validated;
    }
    generateMainBody(params) {
    ...
    createDoor(params, position, size) {
    ...
    createHandle(style, height, finish) {
        let geometry;
        switch (style) {
            case 'bar':
    ...
    createEmissiveMaterial(color, intensity) {
        return this.createPBRMaterial({
            color: 0x111111,
            emissive: color,
    ...
    getBoundingBox(params) {
        return new BBox({ x: -params.width / 2, y: 0, z: -params.depth / 2 }, { x: params.width / 2, y: params.height, z: params.depth / 2 });
    }
    ...
    getRandomParams() {
        const styles = ['modern', 'retro', 'industrial', 'minimal'];
        const finishes = ['stainless', 'black', 'white', 'colored'];
        const handleStyles = ['bar', 'tube', 'recessed', 'knob'];
        return {
...
//# sourceMappingURL=ApplianceBase.js.map