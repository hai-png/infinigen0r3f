...
export class CabinetGenerator extends BaseObjectGenerator {
    ...
    createDoors(params, rng) {
        ...
        for (let i = 0; i < params.doorCount; i++) {
            ...
            const handle = new THREE.Mesh(handleGeom, handleMat);
            handle.position.set(i === 0 ? doorWidth / 2 - 0.03 : -doorWidth / 2 + 0.03, 0, params.hasGlassDoors ? 0.025 : 0.015);
            door.add(handle);
    ...
    createInteriorShelves(params, rng) {
    ...
    getVariationCount() { return 4 * 4 * 3 * 4 * 2 * 2; }
    register() { ObjectRegistry.register(CabinetGenerator.GENERATOR_ID, this); }
}
CabinetGenerator.GENERATOR_ID = 'cabinet_generator';
if (typeof window !== 'undefined') {
    new CabinetGenerator().register();
}
export default CabinetGenerator;
//# sourceMappingURL=CabinetGenerator.js.map