...
export class GravelGenerator {
     ...
     */
    generateGravelInstanced(config = {}) {
        const finalConfig = {
            sizeMin: 0.05,
            sizeMax: 0.15,
            count: 500,
        ...
        const baseGeometry = this.createGravelGeometry((finalConfig.sizeMin + finalConfig.sizeMax) / 2);
        // Create material
    ...
    generateGravelPath(config) {
        ...
        for (let i = 0; i < pathConfig.count && instanceIndex < pathConfig.count; i++) {
            ...
            dummy.rotation.set(Math.random() * 0.3, // Slight tilt
            Math.random() * Math.PI * 2, Math.random() * 0.3);
            dummy.updateMatrix();
            instancedMesh.setMatrixAt(instanceIndex++, dummy.matrix);
        }
        instancedMesh.instanceMatrix.needsUpdate = true;
    ...
    generateGravelBorder(config) {
        const { innerRadius, outerRadius, arcAngle = Math.PI * 2, segmentCount = 20, } = config;
        ...
        for (let i = 0; i < segmentCount; i++) {
            ...
            gravelCluster.position.set(Math.cos(angle) * radius, 0, Math.sin(angle) * radius);
...
//# sourceMappingURL=GravelGenerator.js.map