...
(function (LeafType) {
    ...
    LeafType["BIRCH"] = "birch";
    LeafType["PINE"] = "pine";
    LeafType["MIXED"] = "mixed";
})(LeafType || (LeafType = {}));
/**
 * Decomposition states for realistic leaf aging
 */
export var DecompositionState;
(function (DecompositionState) {
    DecompositionState["FRESH"] = "fresh";
    DecompositionState["DRYING"] = "drying";
    DecompositionState["DECAYING"] = "decaying";
    DecompositionState["HUMUS"] = "humus"; // Nearly decomposed, dark
...
export class LeafLitterGenerator {
    ...
    static generate(config) {
        ...
        for (let i = 0; i < leafCount; i++) {
            ...
            dummy.rotation.set((Math.random() - 0.5) * 0.3, rotationY, (Math.random() - 0.5) * 0.3);
            // Scale variation
     ...
     */
    static createLeafGeometry(leafType) {
     ...
     */
    static generateMultiLayer(config, layers) {
        const group = new THREE.Group();
    ...
    static getLayerDecomposition(layerIndex, totalLayers) {
        const ratio = layerIndex / totalLayers;
        if (ratio < 0.25)
            return DecompositionState.FRESH;
        if (ratio < 0.5)
            return DecompositionState.DRYING;
        if (ratio < 0.75)
            return DecompositionState.DECAYING;
        return DecompositionState.HUMUS;
    }
}
LeafLitterGenerator.LEAF_SHAPES = {
    [LeafType.OAK]: [
...
//# sourceMappingURL=LeafLitterGenerator.js.map