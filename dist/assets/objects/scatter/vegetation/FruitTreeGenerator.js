...
export class FruitTreeGenerator extends BaseObjectGenerator {
    ...
    createFruits(config, rng) {
        const geom = new THREE.SphereGeometry(0.08, 8, 8);
        let color = 0xff0000;
        if (config.fruitType === 'orange')
            color = 0xffa500;
        else if (config.fruitType === 'cherry')
            color = 0x8b0000;
        else if (config.fruitType === 'peach')
            color = 0xffcba4;
        else if (config.fruitType === 'pear')
            color = 0xd4e157;
...
//# sourceMappingURL=FruitTreeGenerator.js.map