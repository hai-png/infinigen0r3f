...
import { memoKey } from './eval-memo.js';
 ...
 */
function computeNodeVal(node, state, memo) {
    // Handle special case nodes
    if (node instanceof SceneConstant) {
        return new Set(Array.from(state.objs.entries())
    ...
    if (implFunc) {
        const childVals = new Map(Array.from(node.children().entries()).map(([name, child]) => [
            name,
            evaluateNode(child, state, memo)
        ]));
        const kwargs = {};
        if ('othersTags' in node) {
            kwargs.othersTags = node.othersTags;
        }
    ...
    throw new NotImplementedError(`Couldnt compute value for ${node.constructor.name}. Please add it to nodeImpls or add a special case.`);
}
/**
 * Check if a node is relevant to a given domain filter
 */
export function relevant(node, filter) {
    if (filter === null) {
        return true;
    }
 ...
 */
function violCountBinopInteger(node, lhs, rhs) {
 ...
 */
function violCountBinop(node, lhs, rhs) {
 ...
 */
export function violCount(node, state, memo, filter = null) {
    let res;
    // Handle conjunctions and problems
    if ((node instanceof BoolOperatorExpression && node.func === ((a, b) => a && b)) ||
        node instanceof Problem) {
        const constraints = node instanceof Problem ? node.constraints : node.operands;
        res = constraints.reduce((sum, c) => sum + violCount(c, state, memo, filter), 0);
    }
    // Handle in_range
    ...
    // Handle comparison operators
        ...
        const func = node.func;
        const isComparison = func === ((a, b) => a >= b) ||
            func === ((a, b) => a <= b) ||
            func === ((a, b) => a > b) ||
            func === ((a, b) => a < b);
        if (isComparison) {
    ...
    // Default: evaluate normally
        ...
        throw new NotImplementedError(`${node.constructor.name}(...) is not supported for hard constraints. Please use an alternative.`);
    }
    return res;
}
/**
 ...
 */
export function evaluateNode(node, state, memo = null) {
...
export class EvalResult {
    constructor(lossVals, violations) {
 ...
 */
export function evaluateProblem(problem, state, filter = null, memo = null, enableLoss = true, enableViolated = true) {
    if (memo === null) {
        memo = new Map();
    }
    const lossVals = new Map();
