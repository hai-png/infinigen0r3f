 ...
 * - js-libcsg
 */
import { BufferGeometry, Vector3, Box3, Triangle, Ray, } from 'three';
/**
...
export class MeshOperations {
     ...
     */
    static intersection(meshA, meshB, options = {}) {
        const { preserveUVs = true, cleanup = true } = options;
        const warnings = [];
        try {
            const geomA = meshA.geometry.clone();
     ...
     */
    static difference(meshA, meshB, options = {}) {
        const { preserveUVs = true, cleanup = true } = options;
        const warnings = [];
        try {
            const geomA = meshA.geometry.clone();
     ...
     */
    static simplify(geometry, options) {
        const { targetFaceCount, aggressiveness = 5, preserveBorders = true, preserveUVs = true, } = options;
        ...
        const cellSize = new Vector3(size.x / gridSize, size.y / gridSize, size.z / gridSize);
        // Cluster vertices
        const vertexClusters = new Map();
        ...
        result.setAttribute('position', new Float32Array(newPositions).buffer);
     ...
     */
    static subdivide(geometry, options) {
     ...
     * Voxelize a mesh
     */
    static voxelize(mesh, options) {
        const { resolution, solid = true, includeInterior = false, } = options;
     ...
     */
    static cleanupGeometry(geometry) {
        const positions = geometry.getAttribute('position');
        const indices = geometry.getIndex();
        if (!indices || !positions)
            return;
        ...
        for (let i = 0; i < indexArray.length; i += 3) {
            ...
            if (v0 !== v1 && v1 !== v2 && v0 !== v2) {
                ...
                const p0 = new Vector3(positionArray[v0 * 3], positionArray[v0 * 3 + 1], positionArray[v0 * 3 + 2]);
                const p1 = new Vector3(positionArray[v1 * 3], positionArray[v1 * 3 + 1], positionArray[v1 * 3 + 2]);
                const p2 = new Vector3(positionArray[v2 * 3], positionArray[v2 * 3 + 1], positionArray[v2 * 3 + 2]);
     ...
     */
    static voxelBasedBoolean(geomA, geomB, operation, options) {
     ...
     */
    static createVoxelGrid(geometry, resolution) {
     ...
     */
    static voxelizeTriangle(p0, p1, p2, bbox, cellSize, resolution, voxels) {
        // Calculate triangle bounds
        const min = new Vector3(Math.min(p0.x, p1.x, p2.x), Math.min(p0.y, p1.y, p2.y), Math.min(p0.z, p1.z, p2.z));
        const max = new Vector3(Math.max(p0.x, p1.x, p2.x), Math.max(p0.y, p1.y, p2.y), Math.max(p0.z, p1.z, p2.z));
        // Expand bounds slightly
        min.subScalar(cellSize * 0.5);
        max.addScalar(cellSize * 0.5);
        // Convert to voxel coordinates
        const vmin = new Vector3(Math.max(0, Math.floor((min.x - bbox.min.x) / cellSize)), Math.max(0, Math.floor((min.y - bbox.min.y) / cellSize)), Math.max(0, Math.floor((min.z - bbox.min.z) / cellSize)));
        const vmax = new Vector3(Math.min(resolution - 1, Math.floor((max.x - bbox.min.x) / cellSize)), Math.min(resolution - 1, Math.floor((max.y - bbox.min.y) / cellSize)), Math.min(resolution - 1, Math.floor((max.z - bbox.min.z) / cellSize)));
        ...
        for (let x = vmin.x; x <= vmax.x; x++) {
            for (let y = vmin.y; y <= vmax.y; y++) {
                for (let z = vmin.z; z <= vmax.z; z++) {
                    const voxelCenter = new Vector3(bbox.min.x + (x + 0.5) * cellSize, bbox.min.y + (y + 0.5) * cellSize, bbox.min.z + (z + 0.5) * cellSize);
     ...
     */
    static voxelsToGeometry(voxels, resolution, referenceGeom, preserveUVs) {
        const positions = [];
        const normals = [];
        const uvs = [];
        const indices = [];
        ...
        const bbox = new Box3().setFromAttribute(referenceGeom.getAttribute('position'));
        const size = new Vector3();
        bbox.getSize(size);
        const cellSize = Math.max(size.x, size.y, size.z) / resolution;
        let vertexCount = 0;
        const voxelCenters = new Map();
     ...
     */
    static subdivideOnce(geometry, smoothNormals, preserveUVs) {
        ...
        for (let i = 0; i < positions.count; i++) {
            newPositions.push(positionArray[i * 3], positionArray[i * 3 + 1], positionArray[i * 3 + 2]);
            vertexCount++;
        }
        const newIndices = [];
        ...
        for (let i = 0; i < indexArray.length; i += 3) {
            ...
            const m01 = this.getOrCreateMidpoint(v0, v1, positionArray, edgeMidpoints, newPositions);
            const m12 = this.getOrCreateMidpoint(v1, v2, positionArray, edgeMidpoints, newPositions);
            const m20 = this.getOrCreateMidpoint(v2, v0, positionArray, edgeMidpoints, newPositions);
        ...
        const result = new BufferGeometry();
        result.setAttribute('position', new Float32Array(newPositions));
        result.setIndex(newIndices);
        if (smoothNormals) {
            result.computeVertexNormals();
        }
     ...
     */
    static getOrCreateMidpoint(v0, v1, positionArray, edgeMidpoints, newPositions) {
     ...
     */
    static isPointNearSurface(point, mesh, threshold) {
...
MeshOperations.EPSILON = 1e-10;
export default MeshOperations;
//# sourceMappingURL=MeshOperations.js.map