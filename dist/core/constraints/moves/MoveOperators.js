...
export class PoseMoveOperator {
    ...
    execute(move) {
        ...
        // Apply new pose
        if (move.position)
            obj.position.copy(move.position);
        if (move.rotation)
            obj.rotation.copy(move.rotation);
        if (move.scale)
            obj.scale.copy(move.scale);
        return {
            success: true,
            energyChange: 0,
    ...
    undo(move) {
        ...
        if (!obj)
            return false;
        if (move.previousState.position)
            obj.position.copy(move.previousState.position);
        if (move.previousState.rotation)
            obj.rotation.copy(move.previousState.rotation);
        if (move.previousState.scale)
            obj.scale.copy(move.previousState.scale);
        return true;
    }
}
/**
...
export class AddMoveOperator {
    ...
    execute(move) {
        ...
        if (move.rotation)
            newObj.rotation.copy(move.rotation);
        if (move.scale)
            newObj.scale.copy(move.scale);
...
export class DeleteMoveOperator {
    ...
    undo(move) {
        ...
        newObj.name = move.objectId;
        if (move.previousState.position)
            newObj.position.copy(move.previousState.position);
        if (move.previousState.rotation)
            newObj.rotation.copy(move.previousState.rotation);
        if (move.previousState.scale)
            newObj.scale.copy(move.previousState.scale);
        this.domain.objects.set(move.objectId, newObj);
...
//# sourceMappingURL=MoveOperators.js.map