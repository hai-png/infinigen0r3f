 ...
 */
export function createBoundFromComparison(opFunc, lhs, rhs) {
    const lhsVal = isConstant(lhs) ? lhs() : undefined;
    const rhsVal = isConstant(rhs) ? rhs() : undefined;
 ...
 */
export function mapBound(bound, func, lhs, rhs) {
 ...
 */
export function expressionMapBoundBinop(node, bound) {
    ...
    if (lhsConst && !rhsConst) {
        const lhsVal = lhs();
        return expressionMapBound(rhs, mapBound(bound, (a, b) => {
            switch (invFunc) {
                case 'sub': return a - b;
                case 'add': return a + b;
    ...
    if (!lhsConst && rhsConst) {
        const rhsVal = rhs();
        return expressionMapBound(lhs, mapBound(bound, (a, b) => {
            switch (invFunc) {
                case 'sub': return a + b;
                case 'add': return a - b;
                case 'div': return a * b;
 ...
 */
export function expressionMapBound(node, bound) {
 ...
 */
export function evaluateKnownVars(node, knownVars) {
    if (isConstant(node)) {
        return null;
    }
    if (node instanceof ScalarOperatorExpression) {
        ...
        if (intInverseOp[node.func]) {
            if (isConstant(lhs)) {
                ...
                if (rhsEval !== null) {
                    return node.func(lhs(), rhsEval);
                }
            }
            else {
                const lhsEval = evaluateKnownVars(lhs, knownVars);
                if (lhsEval !== null) {
                    return node.func(lhsEval, rhs());
                }
            }
        }
    }
 ...
 */
export function constraintBounds(node, state) {
    const recurse = (n) => constraintBounds(n, state);
    ...
    if (node.func === 'inRange') {
        ...
        const bound = { low: lowVal, high: highVal };
 ...
 */
function updateVar(varNode, state) {
 ...
 */
export function isValidBound(bound) {
 ...
 */
export function satisfiesBound(value, bound) {
...
//# sourceMappingURL=constraint-bounding.js.map