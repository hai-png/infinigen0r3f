 ...
 */
export function interpolatePosition(start, end, t) {
    return new THREE.Vector3().lerpVectors(start, end, t);
}
/**
 * Bezier interpolation with control points
 */
export function bezierInterpolate(p0, p1, p2, p3, t) {
    const mt = 1 - t;
    const mt2 = mt * mt;
    const mt3 = mt2 * mt;
    const t2 = t * t;
 ...
 */
export function generateTrajectory(keyframes, config = { keyframes, interpolation: InterpolationMode.CatmullRom }) {
 ...
 */
function sampleAtTime(keyframes, time, mode) {
    if (keyframes.length === 0)
        return null;
    ...
    // Interpolate target
    const target = new THREE.Vector3().lerpVectors(prev.target || new THREE.Vector3(0, 0, 0), next.target || new THREE.Vector3(0, 0, 0), easedT);
 ...
 */
export function createCurveFromTrajectory(samples) {
 ...
 */
export function calculateTrajectoryLength(samples) {
    if (samples.length < 2)
        return 0;
    let length = 0;
 ...
 */
export function resampleUniform(samples, segmentLength) {
    if (samples.length < 2)
        return samples;
...
//# sourceMappingURL=TrajectoryGenerator.js.map