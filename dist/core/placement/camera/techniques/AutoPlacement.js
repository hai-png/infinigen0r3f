...
export function calculateOptimalCameraPosition(subjectPosition, subjectBounds, options = {}) {
    const center = subjectBounds.getCenter(new THREE.Vector3());
    const size = subjectBounds.getSize(new THREE.Vector3());
    const diagonal = size.length();
    const distance = Math.max(options.minDistance || diagonal * 2, Math.min(options.maxDistance || diagonal * 5, diagonal * 3));
    ...
    if (options.ruleOfThirds) {
        ...
        return new THREE.Vector3(center.x + Math.cos(angle) * distance + offset, center.y + height, center.z + Math.sin(angle) * distance);
    }
    return new THREE.Vector3(center.x + Math.cos(angle) * distance, center.y + height, center.z + Math.sin(angle) * distance);
}
export function evaluateViewpointQuality(cameraPosition, subjectPosition, sceneObjects) {
    let score = 1.0;
...
//# sourceMappingURL=AutoPlacement.js.map