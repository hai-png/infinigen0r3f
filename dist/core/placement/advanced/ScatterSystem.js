...
import { AdvancedPlacer, createDefaultConfig } from './AdvancedPlacer';
// ============================================================================
...
export class DensityMapGenerator {
     ...
     */
    getValue(worldPos) {
        const x = Math.floor(((worldPos.x - this.bounds.min.x) / (this.bounds.max.x - this.bounds.min.x)) *
            (this.resolution[0] - 1));
        const y = Math.floor(((worldPos.z - this.bounds.min.z) / (this.bounds.max.z - this.bounds.min.z)) *
            (this.resolution[1] - 1));
     ...
     */
    createGradient(direction, startValue, endValue) {
     ...
     */
    createRadial(centers, maxRadius, falloff = 'linear') {
        // Clear values
        this.values.fill(0);
...
export class ClumpingSystem {
    ...
    generateClumpCenters() {
        ...
        for (let i = 0; i < this.config.numClumps; i++) {
            ...
            this.clumpCenters.push(new Vector3(Math.cos(angle) * radius, 0, Math.sin(angle) * radius));
        }
    }
    /**
     * Generate positions with clumping behavior
     */
    generatePositions(totalCount) {
        const positions = [];
        ...
        for (const center of this.clumpCenters) {
            for (let i = 0; i < instancesPerClump; i++) {
                ...
                const pos = new Vector3(center.x + Math.cos(angle) * r, 0, center.z + Math.sin(angle) * r);
                positions.push(pos);
            }
        }
...
export class VariationEngine {
    constructor(scaleRange, rotationVariation, meshVariants = []) {
     ...
     */
    generateRotation(alignToSurface, normal, seed) {
     ...
     */
    generateInstance(position, normal, seed) {
        return {
...
export class LODManager {
     ...
     */
    updateLODs(instances, cameraPosition) {
        let visibleCount = 0;
...
export class ScatterSystem {
    constructor(options) {
        ...
        this.variationEngine = new VariationEngine(this.config.scaleRange, this.config.rotationVariation, this.config.meshVariants || []);
    ...
    async scatter(cameraPosition) {
        ...
        if (this.config.distributionMap) {
            basePositions = basePositions.filter(pos => {
                const density = this.config.distributionMap.values[Math.floor(((pos.x - this.bounds.min.x) / (this.bounds.max.x - this.bounds.min.x)) *
 ...
 */
export async function quickScatter(count, bounds, options) {
...
//# sourceMappingURL=ScatterSystem.js.map