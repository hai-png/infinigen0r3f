 ...
 */
export function createBumpGroup(config) {
    const { wrangler, location = [0, 0] } = config;
    const group = wrangler.createNodeGroup('Bump');
    const groupId = group.id;
    // Create nodes
    const bumpNode = wrangler.newNode(NodeTypes.BUMP, 'Bump', [location[0], location[1]], { strength: 1.0, distance: 1.0 });
    const normalOutput = wrangler.newNode(NodeTypes.OUTPUT_NORMAL, 'Normal Output', [location[0] + 300, location[1]]);
    // Connect nodes
 ...
 */
export function createNormalMapGroup(config) {
    const { wrangler, location = [0, 0] } = config;
    const group = wrangler.createNodeGroup('NormalMap');
    const groupId = group.id;
    const normalMapNode = wrangler.newNode(NodeTypes.NORMAL_MAP, 'Normal Map', [location[0], location[1]], { strength: 1.0 });
    const output = wrangler.newNode(NodeTypes.OUTPUT_NORMAL, 'Normal Output', [location[0] + 300, location[1]]);
 ...
 */
export function createColorRampGroup(config) {
    const { wrangler, location = [0, 0] } = config;
    const group = wrangler.createNodeGroup('ColorRamp');
    const groupId = group.id;
    const colorRampNode = wrangler.newNode(NodeTypes.COLOR_RAMP, 'Color Ramp', [location[0], location[1]], {
        elements: [
    ...
    const output = wrangler.newNode(NodeTypes.OUTPUT_COLOR, 'Color Output', [location[0] + 300, location[1]]);
 ...
 */
export function createTexCoordGroup(config) {
    const { wrangler, location = [0, 0] } = config;
    const group = wrangler.createNodeGroup('TexCoordTransform');
    const groupId = group.id;
    const texCoordNode = wrangler.newNode(NodeTypes.TEX_COORD, 'Texture Coordinate', [location[0], location[1]], { generated: false });
    const mappingNode = wrangler.newNode(NodeTypes.MAPPING, 'Mapping', [location[0] + 200, location[1]], {
        translation: [0, 0, 0],
        rotation: [0, 0, 0],
        scale: [1, 1, 1],
    });
    const output = wrangler.newNode(NodeTypes.OUTPUT_VECTOR, 'Vector Output', [location[0] + 400, location[1]]);
 ...
 */
export function createNoiseTextureGroup(config) {
    const { wrangler, location = [0, 0] } = config;
    const group = wrangler.createNodeGroup('NoiseTexture');
    const groupId = group.id;
    const noiseNode = wrangler.newNode(NodeTypes.TEX_NOISE, 'Noise Texture', [location[0], location[1]], {
        scale: 5.0,
        detail: 2.0,
        roughness: 0.5,
        distortion: 0.0,
    });
    const colorRampNode = wrangler.newNode(NodeTypes.COLOR_RAMP, 'Color Ramp', [location[0] + 200, location[1]], {
        elements: [
            { position: 0.0, color: [0, 0, 0, 1] },
            { position: 1.0, color: [1, 1, 1, 1] },
        ],
    });
    const output = wrangler.newNode(NodeTypes.OUTPUT_COLOR, 'Color Output', [location[0] + 400, location[1]]);
 ...
 */
export function createPrincipledBSDFGroup(config) {
    const { wrangler, location = [0, 0] } = config;
    const group = wrangler.createNodeGroup('PrincipledBSDF');
    const groupId = group.id;
    const bsdfNode = wrangler.newNode(NodeTypes.BSDF_PRINCIPLED, 'Principled BSDF', [location[0], location[1]], {
    ...
    const output = wrangler.newNode(NodeTypes.OUTPUT_MATERIAL, 'Material Output', [location[0] + 300, location[1]]);
 ...
 */
export function createLayerWeightGroup(config) {
    const { wrangler, location = [0, 0] } = config;
    const group = wrangler.createNodeGroup('LayerWeight');
    const groupId = group.id;
    const layerWeightNode = wrangler.newNode(NodeTypes.LAYER_WEIGHT, 'Layer Weight', [location[0], location[1]], { blend: 0.5 });
    const output = wrangler.newNode(NodeTypes.OUTPUT_VALUE, 'Value Output', [location[0] + 200, location[1]]);
 ...
 */
export function createVectorMathGroup(config) {
    const { wrangler, location = [0, 0] } = config;
    const group = wrangler.createNodeGroup('VectorMath');
    const groupId = group.id;
    const vectorMathNode = wrangler.newNode(NodeTypes.VECTOR_MATH, 'Vector Math', [location[0], location[1]], { operation: 'ADD' });
    const output = wrangler.newNode(NodeTypes.OUTPUT_VECTOR, 'Vector Output', [location[0] + 200, location[1]]);
 ...
 */
export function createPrimitiveGroup(name, wrangler, location) {
...
//# sourceMappingURL=primitive-groups.js.map