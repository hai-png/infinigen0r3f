...
export class NodeTranspiler {
     ...
     */
    generateNodeCode(node, varNames) {
     ...
     */
    transpileToShaderMaterial(tree) {
        const lines = [];
    ...
    optimize(tree) {
        ...
        optimized.nodes.forEach((node, nodeId) => {
            ...
            if (node.type === NodeType.Transform) {
                const isIdentity = JSON.stringify(node.settings.translation) === '[0,0,0]' &&
                ...
                if (isIdentity && inputs.length === 1 && outputs.length === 1) {
                    ...
                    optimized.links = optimized.links.filter(l => l !== inputLink && l !== outputLink);
 ...
 */
export function transpileNodeTree(tree, format = 'typescript') {
