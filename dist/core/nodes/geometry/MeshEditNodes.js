...
export const ExtrudeMeshDefinition = {
 ...
 */
export function executeExtrudeMesh(node, inputMesh) {
    const positions = inputMesh.attributes.position.array;
    const normals = inputMesh.attributes.normal?.array;
    if (!normals) {
    ...
    for (let f = 0; f < faceCount; f++) {
        ...
        const faceNormal = new Vector3((normals[baseIndex] + normals[baseIndex + 3] + normals[baseIndex + 6]) / 3, (normals[baseIndex + 1] + normals[baseIndex + 4] + normals[baseIndex + 7]) / 3, (normals[baseIndex + 2] + normals[baseIndex + 5] + normals[baseIndex + 8]) / 3).normalize();
        ...
        for (let v = 0; v < 3; v++) {
            ...
            newPositions.push(positions[vi] + faceNormal.x * offset, positions[vi + 1] + faceNormal.y * offset, positions[vi + 2] + faceNormal.z * offset);
    ...
    for (let f = 0; f < faceCount; f++) {
        ...
        for (let e = 0; e < 3; e++) {
            ...
            sidePositions.push(positions[v1_orig], positions[v1_orig + 1], positions[v1_orig + 2], positions[v2_orig], positions[v2_orig + 1], positions[v2_orig + 2], positions[v1_orig] + (newPositions[baseExtr + 0] - positions[v1_orig]), positions[v1_orig + 1] + (newPositions[baseExtr + 1] - positions[v1_orig + 1]), positions[v1_orig + 2] + (newPositions[baseExtr + 2] - positions[v1_orig + 2]));
            for (let i = 0; i < 3; i++) {
                sideNormals.push(sideNormal.x, sideNormal.y, sideNormal.z);
            }
            // Triangle 2: v2_orig, v2_extr, v1_extr
            sidePositions.push(positions[v2_orig], positions[v2_orig + 1], positions[v2_orig + 2], positions[v2_orig] + (newPositions[baseExtr + 3] - positions[v2_orig]), positions[v2_orig + 1] + (newPositions[baseExtr + 4] - positions[v2_orig + 1]), positions[v2_orig + 2] + (newPositions[baseExtr + 5] - positions[v2_orig + 2]), positions[v1_orig] + (newPositions[baseExtr + 0] - positions[v1_orig]), positions[v1_orig + 1] + (newPositions[baseExtr + 1] - positions[v1_orig + 1]), positions[v1_orig + 2] + (newPositions[baseExtr + 2] - positions[v1_orig + 2]));
            for (let i = 0; i < 3; i++) {
                sideNormals.push(sideNormal.x, sideNormal.y, sideNormal.z);
            }
        }
    ...
    newGeometry.setAttribute('position', new Float32Array(finalPositions));
    newGeometry.setAttribute('normal', new Float32Array(finalNormals));
    return newGeometry;
}
export const TriangulateDefinition = {
 ...
 */
export function executeTriangulate(node, inputMesh) {
    ...
    if (remainder !== 0) {
        ...
        geometry.setAttribute('position', newArray);
    }
    return geometry;
}
export const MergeByDistanceDefinition = {
 ...
 */
export function executeMergeByDistance(node, inputMesh) {
    const positions = inputMesh.attributes.position.array;
    ...
    const newFacePositions = [];
    for (let i = 0; i < indexMapping.length; i += 3) {
        const i0 = indexMapping[i];
        const i1 = indexMapping[i + 1];
        const i2 = indexMapping[i + 2];
        newFacePositions.push(newPositions[i0 * 3], newPositions[i0 * 3 + 1], newPositions[i0 * 3 + 2], newPositions[i1 * 3], newPositions[i1 * 3 + 1], newPositions[i1 * 3 + 2], newPositions[i2 * 3], newPositions[i2 * 3 + 1], newPositions[i2 * 3 + 2]);
    }
    const newGeometry = new BufferGeometry();
    newGeometry.setAttribute('position', new Float32Array(newFacePositions));
 ...
 */
export function executeTransform(node, inputGeometry) {
    ...
    matrix.makeRotationFromEuler({ x: rotation.x, y: rotation.y, z: rotation.z });
    // Apply translation
...
//# sourceMappingURL=MeshEditNodes.js.map