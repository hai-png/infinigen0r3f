 ...
 */
async function renderFrames(scene, config) {
    const warnings = [];
    const framePaths = [];
    const { outputFolder, frameRange = [1, 1], currentFrame = 1, hideWater = false, resolution = [1280, 720], format = 'png', quality = 0.9 } = config;
    // Handle water hiding
    if (hideWater) {
 ...
 */
export const renderTask = async (scene, config) => {
 ...
 */
export const renderTaskMetadata = {
    name: 'render',
 ...
 */
export function registerRenderTask() {
 ...
 */
export async function executeRender(scene, config) {
