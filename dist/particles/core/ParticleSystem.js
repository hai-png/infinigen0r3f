...
export class Particle {
    ...
    initialize(config) {
        if (config.position)
            this.data.position.copy(config.position);
        if (config.velocity)
            this.data.velocity.copy(config.velocity);
        if (config.acceleration)
            this.data.acceleration.copy(config.acceleration);
        if (config.mass !== undefined)
            this.data.mass = config.mass;
        if (config.size !== undefined)
            this.data.size = config.size;
        if (config.color)
            this.data.color.copy(config.color);
        if (config.alpha !== undefined)
            this.data.alpha = config.alpha;
        if (config.lifetime !== undefined)
            this.data.lifetime = config.lifetime;
        if (config.rotation !== undefined)
            this.data.rotation = config.rotation;
        if (config.rotationSpeed !== undefined)
            this.data.rotationSpeed = config.rotationSpeed;
        if (config.scale)
            this.data.scale.copy(config.scale);
        if (config.textureIndex !== undefined)
            this.data.textureIndex = config.textureIndex;
        if (config.userData)
            this.data.userData = { ...config.userData };
        this.data.age = 0;
        this.data.force.set(0, 0, 0);
        this.data.lifeCycle = 'birth';
    }
    update(deltaTime) {
        if (this.data.lifeCycle === 'dead')
            return;
        // Update age
        ...
        // Integrate velocity
        this.data.velocity.add(this.data.acceleration.clone().multiplyScalar(deltaTime));
        // Apply drag
        const drag = Math.exp(-this.data.userData.dragCoefficient || 0.1 * deltaTime);
        this.data.velocity.multiplyScalar(drag);
        // Integrate position
        this.data.position.add(this.data.velocity.clone().multiplyScalar(deltaTime));
        // Update rotation
    ...
    addImpulse(impulse) {
        this.data.velocity.add(impulse.clone().divideScalar(this.data.mass));
    }
}
// ============================================================================
// Particle Pool
...
export class ParticleEmitter {
    constructor(config, maxParticles) {
        this.config = { ...config };
    ...
    generatePosition() {
        ...
        switch (config.shape) {
            ...
            case 'sphere':
                ...
                break;
            case 'box':
                pos.set((Math.random() - 0.5) * config.width, (Math.random() - 0.5) * config.height, (Math.random() - 0.5) * config.depth);
                break;
            case 'cone':
                {
            ...
            case 'cylinder':
                ...
                break;
            case 'plane':
                pos.set((Math.random() - 0.5) * config.width, 0, (Math.random() - 0.5) * config.height);
                break;
            case 'ring':
                {
                    const angle = Math.random() * Math.PI * 2;
...
export class ParticleSystem {
    ...
    resolveCollision(particle, collider, deltaTime) {
