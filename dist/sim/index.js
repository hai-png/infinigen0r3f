...
export class KinematicChain {
     ...
     */
    computeForwardKinematics(jointPositions) {
     ...
     */
    computeJointTransform(node, position) {
    ...
    computeInertia() {
        ...
        switch (this.collider) {
            case ColliderType.BOX: {
                ...
                return new Vector3((mass / 12) * (height * height + depth * depth), (mass / 12) * (width * width + depth * depth), (mass / 12) * (width * width + height * height));
            }
            ...
            case ColliderType.CYLINDER: {
                ...
                return new Vector3((1 / 12) * mass * (3 * r * r + h * h), (1 / 2) * mass * r * r, (1 / 12) * mass * (3 * r * r + h * h));
            }
            default:
...
export class SimJoint {
    ...
    isWithinLimits(position) {
        ...
        if (limits.lower !== undefined && position < limits.lower)
            return false;
        if (limits.upper !== undefined && position > limits.upper)
            return false;
        return true;
    }
    /**
     ...
     */
    computeLimitForce(position, velocity) {
        const limits = this.config.limits;
        if (!limits)
            return 0;
        let force = 0;
 ...
 */
export function exportToURDF(chain) {
 ...
 */
export function createPhysicsWorldConfig() {
    return {
