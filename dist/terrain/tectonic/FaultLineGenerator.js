...
export class FaultLineGenerator {
     ...
     */
    generateFaultLine(origin, gridSize, resolution) {
        const { faultLength, numSegments, segmentVariation, verticalSlip, horizontalSlip, slipVariation, dipAngle, strikeAngle } = this.params;
        ...
        for (let i = 0; i < numSegments; i++) {
            ...
            const endPoint = new Vector3(currentPosition.x + Math.sin(segmentStrike) * segmentLength, currentPosition.y, currentPosition.z + Math.cos(segmentStrike) * segmentLength);
     ...
     */
    generateFractureZone(segments, cellSize) {
     ...
     */
    generatePressureRidges(segments) {
        const ridges = [];
        for (const segment of segments) {
     ...
     */
    applyDisplacementToElevation(elevationMap, faultLine, gridSize, resolution) {
        const cellSize = resolution / gridSize;
        for (const segment of faultLine.segments) {
            ...
            const faultNormal = new Vector3(normalDir.x * Math.sin(dipRad), Math.cos(dipRad), normalDir.z * Math.sin(dipRad)).normalize();
     ...
     */
    generateOffsetStreams(streams, faultLine) {
        ...
        for (const stream of streams) {
            ...
            if (crossesFault && crossingPoint) {
                ...
                const splitIndex = stream.findIndex(p => p.distanceTo(crossingPoint) < 100 // Within 100m of crossing
                );
                if (splitIndex > 0) {
                    const original = [...stream];
                    const displaced = stream.slice(splitIndex).map(p => {
                        const offset = new Vector3(this.params.horizontalSlip, 0, 0);
    ...
    findIntersection(p1, p2, p3, p4) {
        ...
        if (det === 0)
            return new Vector3();
        const lambda = ((p4.z - p3.z) * (p4.x - p1.x) + (p3.x - p4.x) * (p4.z - p1.z)) / det;
        return new Vector3(p1.x + lambda * (p2.x - p1.x), 0, p1.z + lambda * (p2.z - p1.z));
    }
    /**
     * Render fault line as Three.js geometry
     */
    createFaultGeometry(faultLine) {
        const positions = [];
        const indices = [];
...
//# sourceMappingURL=FaultLineGenerator.js.map