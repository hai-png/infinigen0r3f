...
export class TectonicPlateSimulator {
    ...
    initializePlates(resolution, worldSize) {
        ...
        for (let i = 0; i < this.config.numPlates; i++) {
            ...
            const centroid = new Vector3(Math.cos(angle) * radius, 0, Math.sin(angle) * radius);
            // Random velocity direction
            const velocityAngle = Math.random() * Math.PI * 2;
            const velocity = new Vector3(Math.cos(velocityAngle), 0, Math.sin(velocityAngle)).multiplyScalar(this.config.plateVelocity * (0.5 + Math.random() * 0.5));
        ...
        for (const boundary of this.boundaries) {
            ...
            for (const idx of boundary.cells) {
                ...
                if (distToTrench > 50 && distToTrench < 200) {
                    ...
                    const volcanoPos = new Vector3(x + direction.x * offset, heightmap[idx], z + direction.z * offset);
