...
export class SignedDistanceField {
    constructor(config) {
        ...
        this.voxelSize = new THREE.Vector3(size.x / this.gridSize[0], size.y / this.gridSize[1], size.z / this.gridSize[2]);
     ...
     */
    getPosition(gx, gy, gz) {
        return new THREE.Vector3(this.bounds.min.x + (gx + 0.5) * this.voxelSize.x, this.bounds.min.y + (gy + 0.5) * this.voxelSize.y, this.bounds.min.z + (gz + 0.5) * this.voxelSize.z);
    }
    /**
 ...
 */
export function meshToSDF(geometry, config) {
 ...
 */
function pointToMeshDistance(point, positions, indices) {
 ...
 */
export function sdfBoolean(sdf1, sdf2, operation) {
 ...
 */
export function sdfSmoothUnion(sdf1, sdf2, k = 0.5) {
    const result = new SignedDistanceField({
        resolution: sdf1.resolution,
        bounds: sdf1.bounds,
    });
 ...
 */
export function sdfOffset(sdf, distance) {
    const result = new SignedDistanceField({
 ...
 */
export function extractIsosurface(sdf, isolevel = 0) {
 ...
 */
export function createPrimitiveSDF(type, bounds, resolution, params) {
            ...
            for (let x = 0; x < sdf.gridSize[0]; x++) {
                ...
                switch (type) {
                    ...
                    case 'box':
                        ...
                        const q = new THREE.Vector3(Math.abs(localPos.x), Math.abs(localPos.y), Math.abs(localPos.z)).sub(halfSize);
                        distance = new THREE.Vector3(Math.max(q.x, 0), Math.max(q.y, 0), Math.max(q.z, 0)).length() + Math.min(Math.max(q.x, Math.max(q.y, q.z)), 0);
                        break;
                    case 'cylinder':
...
//# sourceMappingURL=sdf-operations.js.map