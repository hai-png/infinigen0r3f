...
export class RiverNetwork {
     ...
     */
    computeFlowField(heightmap, resolution, worldSize) {
        const flowData = [];
     ...
     */
    traceFlow(startIdx, flowData, heightmap, resolution, visited) {
        let idx = startIdx;
        const path = [];
     ...
     */
    extractRiverPaths(heightmap, resolution, worldSize) {
        if (!this.flowData) {
        ...
        for (const sourceIdx of sources) {
            if (used[sourceIdx])
                continue;
            const riverPath = this.traceRiverPath(sourceIdx, heightmap, resolution, worldSize, used);
     ...
     */
    traceRiverPath(startIdx, heightmap, resolution, worldSize, used) {
        const riverPoints = [];
        const cellSize = worldSize / resolution;
        let idx = startIdx;
        let length = 0;
     ...
     */
    computeMeanderOffset(x, z, width) {
        ...
        return new THREE.Vector3(nx * this.config.meanderIntensity * width, 0, nz * this.config.meanderIntensity * width);
    }
    /**
     * Carve river channel into terrain
     */
    carveRiverChannel(heightmap, rivers, resolution, worldSize) {
     ...
     */
    createDelta(riverEnd, heightmap, resolution, worldSize) {
        const result = new Float32Array(heightmap);
        const cellSize = worldSize / resolution;
     ...
     */
    generate(heightmap, resolution, worldSize) {
        // Compute flow field
        ...
        const carvedTerrain = this.carveRiverChannel(heightmap, rivers, resolution, worldSize);
        ...
        for (const river of rivers) {
            if (river.length > 0) {
                ...
                if (endPoint.position.y < this.config.minElevation + 50) {
                    const deltaTerrain = this.createDelta(endPoint, carvedTerrain, resolution, worldSize);
                    // Merge delta terrain
...
//# sourceMappingURL=RiverNetwork.js.map