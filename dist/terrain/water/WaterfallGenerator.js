...
export class WaterfallGenerator {
     ...
     */
    detectWaterfallLocations(rivers, heightmap, resolution, worldSize) {
        const locations = [];
     ...
     */
    generateWaterfall(location, heightmap, resolution, worldSize) {
        ...
        const plungePool = this.generatePlungePool(new Vector3(currentX, currentY, currentZ), riverPoint.width, heightmap, resolution, worldSize);
     ...
     */
    determineNumTiers(height) {
        if (height < 15)
            return 1;
        if (height < 30)
            return Math.random() < this.config.tierProbability ? 2 : 1;
        if (height < 60)
     ...
     */
    generatePlungePool(basePosition, riverWidth, heightmap, resolution, worldSize) {
        const cellSize = worldSize / resolution;
     ...
     */
    generateMistParticles(tiers, plungePool) {
        const particles = [];
        ...
        for (const tier of tiers) {
            ...
            for (let i = 0; i < particleCount / tiers.length; i++) {
                ...
                particles.push(new Vector3(impactX + Math.cos(angle) * radius, impactY + Math.random() * tier.height * 0.5, impactZ + Math.sin(angle) * radius));
            }
        }
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            particles.push(new Vector3(plungePool.position.x + Math.cos(angle) * radius, plungePool.position.y + Math.random() * plungePool.depth * 0.3, plungePool.position.z + Math.sin(angle) * radius));
        }
        return particles;
    }
    /**
     ...
     */
    carveWaterfall(heightmap, waterfall, resolution, worldSize) {
        ...
        for (const tier of waterfall.tiers) {
            ...
            for (let r = row - widthInCells; r <= row + widthInCells; r++) {
                for (let c = col - overhangInCells; c <= col + widthInCells; c++) {
                    if (c < 0 || c >= resolution || r < 0 || r >= resolution)
                        continue;
                    const idx = r * resolution + c;
     ...
     */
    generate(rivers, heightmap, resolution, worldSize) {
        // Detect potential locations
        const locations = this.detectWaterfallLocations(rivers, heightmap, resolution, worldSize);
        const waterfalls = [];
        let terrain = new Float32Array(heightmap);
        // Generate waterfall at each suitable location
        for (const location of locations) {
            const waterfall = this.generateWaterfall(location, terrain, resolution, worldSize);
            if (waterfall) {
...
//# sourceMappingURL=WaterfallGenerator.js.map