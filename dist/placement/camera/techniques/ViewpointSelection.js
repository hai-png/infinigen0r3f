 ...
 */
export function scoreViewpoint(position, scene, target, config) {
    ...
    const totalScore = weights.visibility * visibilityScore +
 ...
 */
export function evaluateViewpoint(position, scene, target, config) {
 ...
 */
export function selectBestViewpoint(candidates, scene, target, config) {
    ...
    for (const candidate of candidates) {
        ...
        if (config?.constraints) {
            ...
            if (maxDistance && distance > maxDistance)
                continue;
            if (minHeight && candidate.y < minHeight)
                continue;
            if (maxHeight && candidate.y > maxHeight)
                continue;
            if (avoidZones) {
 ...
 */
export function generateViewpointCandidates(target, options = {}) {
    const { radius = target.diagonal() * 2, horizontalSteps = 16, verticalSteps = 8, minElevation = 0.1, maxElevation = 0.9 } = options;
    const candidates = [];
...
// Helper functions
function findSceneCenter(scene) {
    if (scene.length === 0)
        return null;
...
function countVisibleObjects(position, direction, scene, constraints) {
...
function calculateCenterDistance(position, target, direction) {
    const toTarget = new THREE.Vector3().subVectors(target, position).normalize();
    return 1.0 - direction.dot(toTarget);
}
function calculateRuleOfThirdsScore(position, target) {
...
function calculateLeadingLinesScore(position, scene) {
    ...
    for (let i = 0; i < scene.length - 1; i++) {
        if (scene[i].bbox && scene[i + 1].bbox) {
            const dir = new THREE.Vector3().subVectors(scene[i + 1].bbox.center, scene[i].bbox.center).normalize();
            directions.push(dir);
        }
    }
...
function calculateOcclusionRatio(position, target, scene) {
...
function calculateDepthVariation(position, scene) {
    if (scene.length < 2)
        return 0;
    const distances = [];
    for (const obj of scene) {
...
//# sourceMappingURL=ViewpointSelection.js.map