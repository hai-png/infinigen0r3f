/**
 * Vector Nodes - Mathematical vector operations
 * Based on Blender vector math nodes and infinigen geometry processing
 *
 * These nodes handle all vector mathematics needed for procedural generation
 */
import { NodeTypes } from '../core/node-types';
// ============================================================================
// Node Implementations
// ============================================================================
/**
 * Vector Math Node
 * Performs various mathematical operations on vectors
 */
export class VectorMathNode {
    constructor() {
        this.type = NodeTypes.VectorMath;
        this.name = 'Vector Math';
        this.inputs = {
            operation: 'add',
            vector1: [1, 0, 0],
            vector2: [0, 1, 0],
            value: 1,
            scale: 1,
            clamp: false,
        };
        this.outputs = {
            vector: [0, 0, 0],
            value: 0,
        };
    }
    execute() {
        const op = this.inputs.operation || 'add';
        const v1 = this.inputs.vector1 || [0, 0, 0];
        const v2 = this.inputs.vector2 || [0, 0, 0];
        const val = this.inputs.value ?? 1;
        let result = [0, 0, 0];
        let scalarValue = 0;
        switch (op) {
            case 'add':
                result = [v1[0] + v2[0], v1[1] + v2[1], v1[2] + v2[2]];
                break;
            case 'subtract':
                result = [v1[0] - v2[0], v1[1] - v2[1], v1[2] - v2[2]];
                break;
            case 'multiply':
                result = [v1[0] * v2[0], v1[1] * v2[1], v1[2] * v2[2]];
                break;
            case 'divide':
                result = [
                    v2[0] !== 0 ? v1[0] / v2[0] : 0,
                    v2[1] !== 0 ? v1[1] / v2[1] : 0,
                    v2[2] !== 0 ? v1[2] / v2[2] : 0,
                ];
                break;
            case 'cross_product':
                result = [
                    v1[1] * v2[2] - v1[2] * v2[1],
                    v1[2] * v2[0] - v1[0] * v2[2],
                    v1[0] * v2[1] - v1[1] * v2[0],
                ];
                break;
            case 'dot_product':
                scalarValue = v1[0] * v2[0] + v1[1] * v2[1] + v1[2] * v2[2];
                break;
            case 'length':
                scalarValue = Math.sqrt(v1[0] ** 2 + v1[1] ** 2 + v1[2] ** 2);
                break;
            case 'distance':
                scalarValue = Math.sqrt((v1[0] - v2[0]) ** 2 + (v1[1] - v2[1]) ** 2 + (v1[2] - v2[2]) ** 2);
                break;
            case 'normalize':
                const len = Math.sqrt(v1[0] ** 2 + v1[1] ** 2 + v1[2] ** 2);
                result = len > 0 ? [v1[0] / len, v1[1] / len, v1[2] / len] : [0, 0, 0];
                scalarValue = len;
                break;
            case 'scale':
                const s = this.inputs.scale ?? 1;
                result = [v1[0] * s, v1[1] * s, v1[2] * s];
                break;
            case 'project':
                const dot = v1[0] * v2[0] + v1[1] * v2[1] + v1[2] * v2[2];
                const v2LenSq = v2[0] ** 2 + v2[1] ** 2 + v2[2] ** 2;
                if (v2LenSq > 0) {
                    const factor = dot / v2LenSq;
                    result = [v2[0] * factor, v2[1] * factor, v2[2] * factor];
                }
                break;
            case 'reflect':
                const n = this.normalize(v2);
                const d = v1[0] * n[0] + v1[1] * n[1] + v1[2] * n[2];
                result = [v1[0] - 2 * d * n[0], v1[1] - 2 * d * n[1], v1[2] - 2 * d * n[2]];
                break;
            case 'floor':
                result = [Math.floor(v1[0]), Math.floor(v1[1]), Math.floor(v1[2])];
                break;
            case 'ceiling':
                result = [Math.ceil(v1[0]), Math.ceil(v1[1]), Math.ceil(v1[2])];
                break;
            case 'absolute':
                result = [Math.abs(v1[0]), Math.abs(v1[1]), Math.abs(v1[2])];
                break;
            case 'minimum':
                result = [
                    Math.min(v1[0], v2[0]),
                    Math.min(v1[1], v2[1]),
                    Math.min(v1[2], v2[2]),
                ];
                break;
            case 'maximum':
                result = [
                    Math.max(v1[0], v2[0]),
                    Math.max(v1[1], v2[1]),
                    Math.max(v1[2], v2[2]),
                ];
                break;
            case 'sine':
                result = [Math.sin(v1[0]), Math.sin(v1[1]), Math.sin(v1[2])];
                break;
            case 'cosine':
                result = [Math.cos(v1[0]), Math.cos(v1[1]), Math.cos(v1[2])];
                break;
            case 'modulo':
            case 'modulus':
                result = [
                    v1[0] % v2[0],
                    v1[1] % v2[1],
                    v1[2] % v2[2],
                ];
                break;
            case 'fraction':
                result = [
                    v1[0] - Math.floor(v1[0]),
                    v1[1] - Math.floor(v1[1]),
                    v1[2] - Math.floor(v1[2]),
                ];
                break;
            case 'square_root':
                result = [
                    Math.sqrt(Math.abs(v1[0])),
                    Math.sqrt(Math.abs(v1[1])),
                    Math.sqrt(Math.abs(v1[2])),
                ];
                break;
            default:
                result = [...v1];
        }
        if (this.inputs.clamp) {
            result = [
                Math.max(-1, Math.min(1, result[0])),
                Math.max(-1, Math.min(1, result[1])),
                Math.max(-1, Math.min(1, result[2])),
            ];
        }
        this.outputs.vector = result;
        this.outputs.value = scalarValue;
        return this.outputs;
    }
    normalize(v) {
        const len = Math.sqrt(v[0] ** 2 + v[1] ** 2 + v[2] ** 2);
        return len > 0 ? [v[0] / len, v[1] / len, v[2] / len] : [0, 0, 0];
    }
}
/**
 * Vector Rotate Node
 * Rotates a vector using various methods
 */
export class VectorRotateNode {
    constructor() {
        this.type = NodeTypes.VectorRotate;
        this.name = 'Vector Rotate';
        this.inputs = {
            rotationType: 'euler_xyz',
            vector: [1, 0, 0],
            center: [0, 0, 0],
            angle: 0,
            axis: [0, 0, 1],
            quaternion: [1, 0, 0, 0],
            target: [0, 1, 0],
            source: [1, 0, 0],
            pivot: [0, 0, 0],
        };
        this.outputs = {
            vector: [0, 0, 0],
        };
    }
    execute() {
        const v = this.inputs.vector || [0, 0, 0];
        const center = this.inputs.center || [0, 0, 0];
        const type = this.inputs.rotationType || 'euler_xyz';
        // Translate to origin
        const translated = [v[0] - center[0], v[1] - center[1], v[2] - center[2]];
        let rotated;
        switch (type) {
            case 'axis_angle':
                rotated = this.rotateAxisAngle(translated, this.inputs.axis || [0, 0, 1], this.inputs.angle || 0);
                break;
            case 'quaternion':
                rotated = this.rotateQuaternion(translated, this.inputs.quaternion || [1, 0, 0, 0]);
                break;
            case 'direction_to_point':
                rotated = this.rotateDirectionToPoint(translated, this.inputs.source || [1, 0, 0], this.inputs.target || [0, 1, 0]);
                break;
            default:
                rotated = translated;
        }
        // Translate back
        this.outputs.vector = [
            rotated[0] + center[0],
            rotated[1] + center[1],
            rotated[2] + center[2],
        ];
        return this.outputs;
    }
    rotateAxisAngle(v, axis, angle) {
        const normalizedAxis = this.normalize(axis);
        const cos = Math.cos(angle);
        const sin = Math.sin(angle);
        const oneMinusCos = 1 - cos;
        const x = normalizedAxis[0];
        const y = normalizedAxis[1];
        const z = normalizedAxis[2];
        return [
            (oneMinusCos * x * x + cos) * v[0] + (oneMinusCos * x * y - z * sin) * v[1] + (oneMinusCos * x * z + y * sin) * v[2],
            (oneMinusCos * x * y + z * sin) * v[0] + (oneMinusCos * y * y + cos) * v[1] + (oneMinusCos * y * z - x * sin) * v[2],
            (oneMinusCos * x * z - y * sin) * v[0] + (oneMinusCos * y * z + x * sin) * v[1] + (oneMinusCos * z * z + cos) * v[2],
        ];
    }
    rotateQuaternion(v, q) {
        const [qw, qx, qy, qz] = q;
        const [x, y, z] = v;
        return [
            x * (1 - 2 * qy * qy - 2 * qz * qz) + y * (2 * qx * qy - 2 * qz * qw) + z * (2 * qx * qz + 2 * qy * qw),
            x * (2 * qx * qy + 2 * qz * qw) + y * (1 - 2 * qx * qx - 2 * qz * qz) + z * (2 * qy * qz - 2 * qx * qw),
            x * (2 * qx * qz - 2 * qy * qw) + y * (2 * qy * qz + 2 * qx * qw) + z * (1 - 2 * qx * qx - 2 * qy * qy),
        ];
    }
    rotateDirectionToPoint(v, source, target) {
        // Simplified implementation
        return v;
    }
    normalize(v) {
        const len = Math.sqrt(v[0] ** 2 + v[1] ** 2 + v[2] ** 2);
        return len > 0 ? [v[0] / len, v[1] / len, v[2] / len] : [0, 0, 0];
    }
}
/**
 * Combine XYZ Node
 * Combines X, Y, Z components into a vector
 */
export class CombineXYZNode {
    constructor() {
        this.type = NodeTypes.CombineXYZ;
        this.name = 'Combine XYZ';
        this.inputs = {
            x: 0,
            y: 0,
            z: 0,
        };
        this.outputs = {
            vector: [0, 0, 0],
        };
    }
    execute() {
        this.outputs.vector = [
            this.inputs.x || 0,
            this.inputs.y || 0,
            this.inputs.z || 0,
        ];
        return this.outputs;
    }
}
/**
 * Separate XYZ Node
 * Separates a vector into X, Y, Z components
 */
export class SeparateXYZNode {
    constructor() {
        this.type = NodeTypes.SeparateXYZ;
        this.name = 'Separate XYZ';
        this.inputs = {
            vector: [0, 0, 0],
        };
        this.outputs = {
            x: 0,
            y: 0,
            z: 0,
        };
    }
    execute() {
        const v = this.inputs.vector || [0, 0, 0];
        this.outputs.x = v[0];
        this.outputs.y = v[1];
        this.outputs.z = v[2];
        return this.outputs;
    }
}
/**
 * Normalize Node
 * Normalizes a vector to unit length
 */
export class NormalizeNode {
    constructor() {
        this.type = NodeTypes.Normalize;
        this.name = 'Normalize';
        this.inputs = {
            vector: [1, 0, 0],
        };
        this.outputs = {
            vector: [0, 0, 0],
            length: 0,
        };
    }
    execute() {
        const v = this.inputs.vector || [0, 0, 0];
        const length = Math.sqrt(v[0] ** 2 + v[1] ** 2 + v[2] ** 2);
        if (length > 0) {
            this.outputs.vector = [v[0] / length, v[1] / length, v[2] / length];
        }
        else {
            this.outputs.vector = [0, 0, 0];
        }
        this.outputs.length = length;
        return this.outputs;
    }
}
/**
 * Mapping Node
 * Transforms a vector through translation, rotation, and scale
 */
export class MappingNode {
    constructor() {
        this.type = NodeTypes.Mapping;
        this.name = 'Mapping';
        this.inputs = {
            vector: [0, 0, 0],
            translation: [0, 0, 0],
            rotation: [0, 0, 0],
            scale: [1, 1, 1],
            type: 'point',
            useMin: false,
            useMax: false,
            min: [-1, -1, -1],
            max: [1, 1, 1],
        };
        this.outputs = {
            vector: [0, 0, 0],
        };
    }
    execute() {
        let v = [...(this.inputs.vector || [0, 0, 0])];
        // Apply scale
        const scale = this.inputs.scale || [1, 1, 1];
        v = [v[0] * scale[0], v[1] * scale[1], v[2] * scale[2]];
        // Apply rotation (simplified Euler)
        const rot = this.inputs.rotation || [0, 0, 0];
        v = this.rotateEuler(v, rot[0], rot[1], rot[2]);
        // Apply translation
        const trans = this.inputs.translation || [0, 0, 0];
        v = [v[0] + trans[0], v[1] + trans[1], v[2] + trans[2]];
        // Apply clamping
        if (this.inputs.useMin && this.inputs.useMax) {
            const min = this.inputs.min || [-1, -1, -1];
            const max = this.inputs.max || [1, 1, 1];
            v = [
                Math.max(min[0], Math.min(max[0], v[0])),
                Math.max(min[1], Math.min(max[1], v[1])),
                Math.max(min[2], Math.min(max[2], v[2])),
            ];
        }
        this.outputs.vector = v;
        return this.outputs;
    }
    rotateEuler(v, rx, ry, rz) {
        // Apply rotations around X, Y, Z axes
        let result = v;
        // Rotation around X
        if (rx !== 0) {
            const cos = Math.cos(rx);
            const sin = Math.sin(rx);
            result = [
                result[0],
                result[1] * cos - result[2] * sin,
                result[1] * sin + result[2] * cos,
            ];
        }
        // Rotation around Y
        if (ry !== 0) {
            const cos = Math.cos(ry);
            const sin = Math.sin(ry);
            result = [
                result[0] * cos + result[2] * sin,
                result[1],
                -result[0] * sin + result[2] * cos,
            ];
        }
        // Rotation around Z
        if (rz !== 0) {
            const cos = Math.cos(rz);
            const sin = Math.sin(rz);
            result = [
                result[0] * cos - result[1] * sin,
                result[0] * sin + result[1] * cos,
                result[2],
            ];
        }
        return result;
    }
}
// ============================================================================
// Factory Functions
// ============================================================================
export function createVectorMathNode(inputs) {
    const node = new VectorMathNode();
    if (inputs)
        Object.assign(node.inputs, inputs);
    return node;
}
export function createVectorRotateNode(inputs) {
    const node = new VectorRotateNode();
    if (inputs)
        Object.assign(node.inputs, inputs);
    return node;
}
export function createCombineXYZNode(inputs) {
    const node = new CombineXYZNode();
    if (inputs)
        Object.assign(node.inputs, inputs);
    return node;
}
export function createSeparateXYZNode(inputs) {
    const node = new SeparateXYZNode();
    if (inputs)
        Object.assign(node.inputs, inputs);
    return node;
}
export function createNormalizeNode(inputs) {
    const node = new NormalizeNode();
    if (inputs)
        Object.assign(node.inputs, inputs);
    return node;
}
export function createMappingNode(inputs) {
    const node = new MappingNode();
    if (inputs)
        Object.assign(node.inputs, inputs);
    return node;
}
//# sourceMappingURL=VectorNodes.js.map