...
export class NodeSerializer {
     ...
     */
    serializeNodesWithLinks(nodes, links) {
        const serialized = {
            nodes: [],
            links: [],
        };
     ...
     */
    deserializeFromObject(data, options = {}) {
        const opts = {
        ...
        if (data.version !== SERIALIZER_VERSION) {
            console.warn(`Version mismatch: expected ${SERIALIZER_VERSION}, got ${data.version}`);
    ...
    importFromBlender(blenderData) {
        ...
        if (blenderData.nodes) {
            blenderData.nodes.forEach((bNode) => {
                const node = {
        ...
        if (blenderData.links) {
            blenderData.links.forEach((bLink) => {
                tree.links.push({
    ...
    deserializeGroup(data, options) {
        return {
            ...
            interface: {
                inputs: new Map(data.interface.inputs.map(socket => [
                    socket.name,
                    {
                        name: socket.name,
                        type: this.deserializeSocketType(socket.type),
                ...
                outputs: new Map(data.interface.outputs.map(socket => [
                    socket.name,
                    {
                        name: socket.name,
                        type: this.deserializeSocketType(socket.type),
    ...
    deserializeNodeType(typeString) {
        // Map string back to NodeType enum
        // This handles both the enum value and the string representation
        return NodeType[typeString] || typeString;
    }
    deserializeSocketType(typeString) {
        // Map string back to SocketType enum
        return SocketType[typeString] || typeString;
    }
    mapBlenderNodeType(blenderType) {
        ...
        return typeMap[blenderType] || blenderType;
    }
    generateId(prefix) {
 ...
 */
export function serializeNodeTree(tree, pretty = true) {
 ...
 */
export function deserializeNodeTree(jsonString) {
 ...
 */
export function saveToLocalStorage(tree, key) {
...
//# sourceMappingURL=NodeSerializer.js.map